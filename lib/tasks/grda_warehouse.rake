...
namespace :grda_warehouse do
  ...
  task initialize_service_service_homelessness: [:environment, 'log:info_to_stdout'] do
    ...
    GrdaWarehouse::ServiceHistoryService.
      in_project_type(HudHelper.util.homeless_project_types).
      update_all(homeless: true)

    # Clients enrolled in chronic projects are literally homeless
    GrdaWarehouse::ServiceHistoryService.
      in_project_type(HudHelper.util.chronic_project_types).
    ...
    #
    # GrdaWarehouse::ServiceHistoryService.
    #   in_project_type(HudHelper.util.residential_project_type_numbers_by_code[:th]).
    ...
    GrdaWarehouse::ServiceHistoryService.
      ...
      in_project_type(HudHelper.util.residential_project_type_numbers_by_code[:ph]).
  ...
  task hourly: [:environment, 'log:info_to_stdout'] do
    ...
    safely_execute do
      ...
      PurgeSoftDeletedRecordsJob.set(priority: BaseJob::EVENTUAL_CONSISTENCY_PRIORITY).perform_later(dry_run: false) if DateTime.current.hour == 5 && enabled
    end
  ...
  task :warm_cohort_cache, [] => [:environment, 'log:info_to_stdout'] do
    # Queue the cohort analytics generation job if it's not already queued
    GrdaWarehouse::Cohort.delay(queue: ENV.fetch('DJ_LONG_QUEUE_NAME', :long_running), priority: BaseJob::CACHE_UPDATE_PRIORITY).prepare_active_cohorts unless Delayed::Job.queued?('prepare_active_cohorts')
  end
  ...
  task :fake_coc_cleanup, [] => [:environment, 'log:info_to_stdout'] do |_task, _args|
    GrdaWarehouse::FakeData.find_each do |fake|
      ...
      fake.map['CoCCode'].each do |k, code|
        if HudHelper.util.cocs.keys.include?(code)
          fake.map['CoCCode'][k] = code
        else
          fake.map['CoCCode'][k] = HudHelper.util.cocs.keys.sample
        end
      end
      fake.save
    end
