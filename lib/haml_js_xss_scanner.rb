...
class HamlJsXssScanner
  ...
  SAFE_PATTERNS = [
    /\.to_json(\W|.html_safe)/,
    /(^|\W)j[ (]/  # Matches start of line or non-word char followed by 'j' followed by space or paren
  ]
  ...
  def scan_file(file)
    ...
    content.each_line do |line|
      ...
      if in_js_block && !line.strip.empty?
        ...
        if js_indentation >= current_indentation && !line.strip.empty?
          in_js_block = false
        end
      end
...
# Run the scanner
scanner = HamlJsXssScanner.new(options)
scanner.scan
