housed <- read.csv(housed_file_path)
...
returns <- read.csv(returns_file_path)
...
time_to_housing_2 <- paste(round(mean(as.Date(housed_2$search_end) - as.Date(housed_2$search_start), na.rm = TRUE), digits = 2), "days to find housing", sep=" ")
...
time_in_housing_2 <- paste(round(mean(as.Date(housed_2$housing_exit) - as.Date(housed_2$housed_date), na.rm = TRUE), digits = 2), "days in program", sep=" ")
