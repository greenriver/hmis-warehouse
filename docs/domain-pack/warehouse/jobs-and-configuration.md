...
Scheduling: `CollectClientMetricsJob` runs from its own daily cron rake task,
`grda_warehouse:collect_client_metrics`. The job runs
`MetricSnapshotCollector.run_daily_collection` under the `collect_client_metrics_job` advisory
lock, wrapped in `instrument_as_maintenance_task(name: 'daily collection')`. The collector calls
`MetricDefinition.maintain!` first, enqueues
...
- A new phase in `Importing::RunDailyImportsJob` for work that does not depend on service
  ...
  entry instead, as `collect_client_metrics`, `purge_soft_deleted_records`,
  `maintain_cohort_intermediate_data`, and `driver:hmis:auto_exit` do.
