...
zip when both `zip_password` and `encryption_type` are blank; raises if only one is present
(model validations require both together going forward, but legacy rows can predate them) or if
`encryption_type` is unrecognized. `encryption_type` `'zip'` calls
`ZipCloak.encrypt(source:, destination:, password:)` (`app/services/zip_cloak.rb`), which drives
...
The controller re-renders `:index` with `flash.now[:error]` set to the recurrence's validation
messages when `RecurringHmisExport.create` fails to persist (e.g. a `zip_password` without an
`encryption_type`), and again with `'Invalid S3 Configuration'` when `s3_present?` but not
`s3_valid?`. `cancel` destroys the recurrence (soft delete) when the requester owns it or
...
- `encrypt_zip` drives `zipcloak` (via `ZipCloak`) or `7z`. A worker image without those
  binaries produces an S3 upload that fails or is empty, while the in-app download still works.
- `ZipCloak::MAX_PASSWORD_LENGTH` and the control-character rejection exist because zipcloak
  reads its password from a `pty` in canonical line mode; the model validation mirrors the same
  limits so a bad password is rejected at save time, not mid-export.
