...
`wait_time: 2.minutes` every hour and, at hour 23, runs
`CandidatePoolBuilder.call(force_reprocessing: true)` under `CandidatePool.lock_for_maintenance!`.
...
- `lib/tasks/grda_warehouse.rake`: hour-23 full rebuild, hourly `ProcessClientsJob` enqueue.
