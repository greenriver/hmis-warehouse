...
(`<env>/custom_assessments/`, only if the directory exists), `seed_static_forms`
...
- `seed_custom_assessment_form_definitions` references an undefined `file_name` in its
  collision error, so a HUD-identifier clash raises `NameError` rather than the intended message.
