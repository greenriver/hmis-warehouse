...
### Superset `analytics.client_piis`

The Scenic view `analytics.client_piis` (`db/views/analytics_client_piis_v02.sql`) is queried directly by Superset with no Rails layer in between, so it can't route through `PiiProvider`. Instead the view itself joins against `hmis_restricted_records` and replaces `FirstName`, `MiddleName`, `LastName`, `NameSuffix`, and `SSN` with the literal `'Redacted'` when an active restriction row exists for the source client (`restrictable_type = 'Hmis::Hud::Client'`, `restrictable_id` matching the source `Client` id, `deleted_at IS NULL`). `DOB` is not redacted in this view. Row-level security in the separate `superset-sync` repository still governs which clients a given Superset user can see at all — this only affects what the fields show for a row that user can already see.
