...
- `MaYyaReport`'s `details.xlsx.axlsx` — the HTML view (`details.haml`) passes `client_id:` into `reporting_policy_for_project` and is restriction-aware, but the Excel export's equivalent call omits `client_id:`, so restriction never applies to that one export. (`HomelessSummaryReport`, `CoreDemographicsReport`, and `WarehouseReport::Outcomes` are fully wired via `reporting_policy_for_client`/`PiiProvider.restrict`, both html and xlsx — not a gap.)
- The CAS-adjacent reports `warehouse_reports/cas/decision_efficiency`, `warehouse_reports/cas/non_hmis_clients`, `warehouse_reports/cas/chronic_reconciliation`, and `warehouse_reports/youth_follow_ups` render raw client name from plucked hashes with no PII policy of any kind — found during the 2026-08-25 exposure-point audit, not yet wired.
- `warehouse_reports/cohort_changes/index.xlsx.axlsx` (a "Youth" cohort-changes export, distinct from the cohorts change report at `app/views/cohorts/reports`) gates client name only on the `include_pii_in_detail_downloads` toggle, with no restriction wrap at all — found during a 2026-08-26 final-review pass, not yet wired.
- `warehouse_reports/health_emergency/testing_results/index.xlsx.axlsx` and `vaccinations/index.xlsx.axlsx` — their HTML `_table.haml` siblings were wired for restriction, but these Excel exports remain gated only by the `include_pii_in_detail_downloads` toggle, with no restriction wrap — found during the same 2026-08-26 pass, not yet wired.
