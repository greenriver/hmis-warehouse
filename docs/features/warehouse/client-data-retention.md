...
`GrdaWarehouse::ClientRetentionMark.rollup_activity` computes, per destination client, one activity
...
1. Creates a `GrdaWarehouse::ClientRetentionRun` with the global window and a snapshot of the data
   source overrides.
2. Evaluates destinations in batches, inserting one `client_retention_marks` row per source client of
...
A source client in `client_retention_marks`, and every destination it is linked to through a live
...
leaves existing marks in place; clear them from `client_retention_marks` in the console.
...
- The OP HMIS frontend does not yet honour these marks. To extend it:
  ...
  `client_retention_marks` with no permission override (a per-id or preloaded lookup added to
  `Hmis::AuthPolicies::ContextLoaders::RestrictedClientLoader`), and `Hmis::Hud::Client.searchable_to`
  should anti-join `client_retention_marks`.
