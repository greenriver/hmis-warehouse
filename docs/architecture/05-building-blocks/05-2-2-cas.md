...
The decomposition follows the referral lifecycle — a match is evaluated, routed through multi-stakeholder approval, and its outcome synced back — with each stage owned by a distinct engine so the matching algorithm, eligibility rules, and approval workflow can change independently. Warehouse Integration is isolated as its own component because the direct-database coupling it carries is the primary driver for consolidation.
