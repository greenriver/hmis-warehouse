...
The components below are split along the referral lifecycle — a match between a client and an available resource is evaluated by the Matching Engine against the eligibility rules, made, then routed through a multi-stakeholder, multi-state approval workflow — with each stage owned by a distinct component so the matching algorithm, the eligibility rules, and the approval workflow can change independently. Warehouse Integration is a component of its own because it holds the outbound coupling: it syncs each outcome back to the Warehouse for reporting over a direct-database API.
...
- **Authentication**: Devise (self-contained local accounts; not behind the shared SSO Authentication Layer)
