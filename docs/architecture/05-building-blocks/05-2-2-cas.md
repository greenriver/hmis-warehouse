...
The components below are split along the referral lifecycle. A match between a client and an available resource is evaluated by the Matching Engine against the eligibility rules, made, and then routed through a multi-stakeholder, multi-state approval workflow. Its outcome is synced back to the Warehouse for reporting by the Warehouse Integration component, over a loose direct-database API.
