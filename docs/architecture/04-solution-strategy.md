...
| --- | --- | --- |
| **Extensibility & Local Configuration** | Each HUD report is its own driver module, and each HUD CSV specification year has its own importer driver, so a new or revised mandate is added alongside the existing ones instead of changing them. Forms and Coordinated Entry rules are stored as definitions an administrator can edit, so a community can collect and report on what it needs without a code change. Warehouse source tables mirror the HUD CSV column for column, so taking on a standards revision means adding or changing the columns HUD changed, not remodeling the data and reworking a mapping layer (Q-1–Q-6). | [2.3 Conventions](02-constraints.md), [5.2.1 Warehouse](05-building-blocks/05-2-1-warehouse.md), [8.1 HMIS Data Model](08-concepts/08-1-hmis-data-model.md) |
| **Data Integrity & Auditability** | Source-preserving warehouse keeps ingested data in its original form; deduplication links rather than discards, and corrected exports replace their source records. The reporting engine snapshots contributing records at generation time, superseded fiscal-year display logic is retained, and application deletes are soft deletes, so figures and records stay recoverable (Q-7–Q-13). | [5.2.1 Warehouse](05-building-blocks/05-2-1-warehouse.md) |
| **Client Protection & Fair Access** | Policy-based authorization makes disclosure permission-checked and deny-by-default, with community policy configured on top of that default; CoC partitioning, access logging and granular access controls mitigate against account compromise. Resource allocation outcomes are auditable: the Coordinated Entry path stores each match, the declining program, and its stated reason, and reports them by reason and agency (Q-14–Q-20). | [5.2.3 Authentication](05-building-blocks/05-2-3-authentication.md), [8.2 Security](08-concepts/08-2-security.md), [5.2.2 CAS](05-building-blocks/05-2-2-cas.md) |
