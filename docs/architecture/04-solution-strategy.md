...
| **Data Integrity & Auditability** | Source-preserving warehouse keeps ingested data in its original form; deduplication links rather than discards, and corrected exports replace their source records. The reporting engine snapshots contributing records at generation time, superseded fiscal-year display logic is retained, and application deletes are soft deletes, so a figure stays reproducible after its inputs move on (Q-7–Q-13). | [5.2.1 Warehouse](05-building-blocks/05-2-1-warehouse.md) |
