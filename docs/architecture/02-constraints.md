...
| **PII Protection** | Client PII must be encrypted at rest and in transit, and all access to it must be auditable. *(The permission model that decides who may see what is [quality goal 3](01-introduction.md#12-quality-goals), not a constraint.)* |
...
| **Small Engineering Team** | Staffing is fixed and small relative to the number of installations served, so the architecture must favor convention and configuration over custom development per community. |
...
| **Imported Data Preserved As-Is** | Data received from external HMISs is preserved as-is — the platform does not correct or sanitize imported records. Reports surface upstream data quality issues intentionally, giving HMIS Leads visibility into DQ problems at their source rather than masking them. |
...
| **Driver Module Pattern** | Features are isolated as self-contained module directories under `/drivers/`, which is how the Multi-Installation Customization constraint above is satisfied without forking. See [8.3 Driver Module Pattern](08-concepts/08-3-driver-module-pattern.md). |
