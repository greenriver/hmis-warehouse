...
# ADR 0004 [Keycloak, Dex, and OAuth2-Proxy as a Single Authentication Mechanism]

## Status

- Current Status: Approved (revised)
...
Keycloak uses **realms** as its primary isolation boundary — a realm per installation or per customer org with a delegated realm admin satisfies the "let an org manage its own users" requirement. Newer Keycloak versions also add an **Organizations** feature for multi-org management within a single realm. This model is already proven on other Green River applications.
...
Migrating from in-app authentication to Keycloak can be accomplished via realm/user import, the Admin REST API, or a User Storage SPI to federate the existing database during cutover. Devise's bcrypt hashes are not natively supported (Keycloak uses PBKDF2), but a third-party bcrypt password-hash provider extension can validate them on import and transparently re-hash to Keycloak's default on first login. Best case scenario, we could migrate all data seamlessly en masse and logins would simply shift. Worst-case scenario, we would need to write post-login workflows to explain and facilitate the change-over on a per-account basis. This is currently untested at Green River.

Using JWT to determine the current user will also require effort, but should result in significantly less application code. Of note, we would need to re-write the masquerade/become/impersonate feature. Keycloak offers an impersonation capability via token exchange, but as of the 26.x line it remains preview/legacy and in active flux (behavior changed across 26.x minor versions; v1 fine-grained-authz removal planned for 27). Keycloak's separate admin-console "impersonate user" action is stable and may be a more dependable building block. The masquerade re-implementation should be validated against the pinned Keycloak version and not assumed stable across upgrades.
