# frozen_string_literal: true

###
# Copyright 2016 - 2025 Green River Data Analysis, LLC
#
# License detail: https://github.com/greenriver/hmis-warehouse/blob/production/LICENSE.md
###

# frozen_string_literal: true

module PerformanceDashboard::Household::Exiting::Coc
  extend ActiveSupport::Concern

  # NOTE: always count the most-recently started enrollment within the range
  def exiting_by_coc
    @exiting_by_coc ||= Rails.cache.fetch([self.class.name, cache_slug, __method__], expires_in: 5.minutes) do
      buckets = coc_buckets.map { |b| [b, []] }.to_h
      counted = {}
      exiting.joins(:enrollment).
        joins(:client).
        order(first_date_in_program: :desc).
        pluck(:household_id, e_t[:enrollment_coc], :first_date_in_program).each do |id, coc, _|
          counted[coc_bucket(coc)] ||= Set.new
          buckets[coc_bucket(coc)] ||= []
          buckets[coc_bucket(coc)] << id unless counted[coc_bucket(coc)].include?(id)
          counted[coc_bucket(coc)] << id
        end
      buckets
    end
  end

  def exiting_by_coc_data_for_chart
    @exiting_by_coc_data_for_chart ||= begin
      columns = [@filter.date_range_words]
      columns += exiting_by_coc.values.map(&:count)
      categories = exiting_by_coc.keys
      filter_selected_data_for_chart(
        {
          labels: categories.map { |s| [s, HudUtility2026.coc_name(s)] }.to_h,
          chosen: @coc_codes,
          columns: columns,
          categories: categories,
        },
      )
    end
  end

  private def exiting_by_coc_details(options)
    sub_key = options[:sub_key]&.to_s
    ids = if sub_key
      exiting_by_coc[sub_key]
    else
      exiting_by_coc.values.flatten
    end
    details = exiting.joins(:client, :enrollment).
      where(household_id: ids).
      order(she_t[:first_date_in_program].desc)
    details = details.where(coc_query(sub_key)) if sub_key
    details.pluck(*detail_columns(options).values).
      map do |row|
        row[-1] = "#{HudUtility2026.coc_name(row.last)} (#{row.last})"
        row
      end.
      index_by(&:first)
  end
end
