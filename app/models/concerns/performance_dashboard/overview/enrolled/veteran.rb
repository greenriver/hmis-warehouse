...
module PerformanceDashboard::Overview::Enrolled::Veteran
  ...
  def enrolled_by_veteran
    @enrolled_by_veteran ||= Rails.cache.fetch([self.class.name, cache_slug, __method__], expires_in: PerformanceDashboards::Overview::EXPIRATION_LENGTH) do
  ...
  def enrolled_by_veteran_data_for_chart
    @enrolled_by_veteran_data_for_chart ||= begin
      ...
      filter_selected_data_for_chart(
        {
          labels: categories.map { |s| [s, HudUtilityCurrent.veteran_status(s)] }.to_h,
