...
module PerformanceDashboard::Overview::Enrolled::Sex
  extend ActiveSupport::Concern

  # NOTE: always count the most-recently started enrollment within the range
  def enrolled_by_sex
    @enrolled_by_sex ||= Rails.cache.fetch([self.class.name, cache_slug, __method__], expires_in: PerformanceDashboards::Overview::EXPIRATION_LENGTH) do
  ...
  def enrolled_by_sex_data_for_chart
    @enrolled_by_sex_data_for_chart ||= begin
      columns = [@filter.date_range_words]
      columns += enrolled_by_sex.values.map(&:count)
      categories = enrolled_by_sex.keys
  ...
  private def enrolled_by_sex_details(options)
    sub_key = options[:sub_key]&.to_i
    ids = if sub_key
      enrolled_by_sex[sub_key]
    else
      enrolled_by_sex.values.flatten
    end
