...
module PerformanceDashboard::Overview::Enrolled::LotHomeless
  ...
  def enrolled_by_lot_homeless_data_for_chart
    @enrolled_by_lot_homeless_data_for_chart ||= begin
      columns = [@filter.date_range_words]
      columns += enrolled_by_lot_homeless.values.map(&:count)
      categories = enrolled_by_lot_homeless.keys
  ...
  private def enrolled_by_lot_homeless_details(options)
    sub_key = options[:sub_key]&.to_sym
    ids = if sub_key
      enrolled_by_lot_homeless[sub_key]
    else
      enrolled_by_lot_homeless.values.flatten
    end
    details = enrolled.joins(:client, :processed_client).
      where(client_id: ids).
