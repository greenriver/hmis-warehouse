...
module PerformanceDashboard::Overview::Exiting::RaceAndEthnicity
  extend ActiveSupport::Concern

  # NOTE: always count the most-recently started enrollment within the range
  def exiting_by_race_and_ethnicity
    @exiting_by_race_and_ethnicity ||= Rails.cache.fetch([self.class.name, cache_slug, __method__], expires_in: PerformanceDashboards::Overview::EXPIRATION_LENGTH) do
  ...
  def exiting_by_race_and_ethnicity_data_for_chart
    @exiting_by_race_and_ethnicity_data_for_chart ||= begin
      columns = [@filter.date_range_words]
      columns += exiting_by_race_and_ethnicity.values.map(&:count)
      categories = exiting_by_race_and_ethnicity.keys
  ...
  private def exiting_by_race_and_ethnicity_details(options)
    sub_key = options[:sub_key]&.to_sym
    ids = if sub_key
      race, ethnicity = sub_key.to_s.split('-')
      exiting_by_race_and_ethnicity[{ race: race, ethnicity: ethnicity.to_sym }]
    else
      exiting_by_race_and_ethnicity.values.flatten
    end
