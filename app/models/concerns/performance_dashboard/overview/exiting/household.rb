...
module PerformanceDashboard::Overview::Exiting::Household # rubocop:disable Style/ClassAndModuleChildren
  ...
  def exiting_by_household_data_for_chart
    @exiting_by_household_data_for_chart ||= begin
      ...
      {
        columns: columns,
        categories: exiting_by_household.keys.map(&:to_s).map(&:humanize),
      }
    end
  end
