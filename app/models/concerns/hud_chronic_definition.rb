module HudChronicDefinition
  ...
  included do
    ...
    def months_12_homeless? on_date:
      date_to_street = service_history.hud_homeless.entry.ongoing(on_date: on_date).order(first_date_in_program: :desc).first&.enrollment&.DateToStreetESSH
    ...
    def times_4_homeless? on_date:
      times_on_street = service_history.hud_homeless.entry.ongoing(on_date: on_date).order(first_date_in_program: :desc).first&.enrollment&.TimesHomelessPastThreeYears
      times_on_street == 4
    end
    ...
    def months_homeless_past_three_years_more_than_12? on_date:
      months_on_street = service_history.hud_homeless.entry.ongoing(on_date: on_date).order(first_date_in_program: :desc).first&.enrollment&.MonthsHomelessPastThreeYears
