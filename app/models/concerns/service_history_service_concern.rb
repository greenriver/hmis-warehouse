...
module ServiceHistoryServiceConcern
  ...
  included do
    ...
    scope :residential, -> {
      where(project_type_column => HudHelper.util.residential_project_type_ids)
    }

    scope :hud_residential, -> do
      hud_project_type(HudHelper.util.residential_project_type_ids)
    end

    scope :residential_non_homeless, -> do
      r_non_homeless = HudHelper.util.residential_project_type_numbers_by_code[:ph] + HudHelper.util.residential_project_type_numbers_by_code[:th]
    ...
    scope :bed_night, -> do
      where(service_type: HudHelper.util.record_type('Bed Night', true))
    end
