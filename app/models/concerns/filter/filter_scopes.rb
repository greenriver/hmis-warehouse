...
module Filter::FilterScopes
  ...
  included do
    ...
    #   :filter_for_funders
    # ] do |id|
    #   define_method(id) do |scope|
    #     #input = Filters::Criteria::INput.new(@filter)
    #     #criteria = Filters::Criteria::Resolver.new(input)
    #     # apply the criteria to the scope
    #     found = @filter.criteria.detect {|c| c.id == id}
    #     found.apply(scope)
    #     #reduce(scope) { |s, c| c.apply(s) }
    #   end
    # end
