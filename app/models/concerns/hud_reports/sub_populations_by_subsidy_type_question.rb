...
module HudReports::SubPopulationsBySubsidyTypeQuestion
  ...
  def sub_populations_by_subsidy_type_question(question:, members:, sub_pops: sub_populations, last_column: 'F')
    ...
    metadata = {
      ...
      row_labels: HmisStructure::Base.current_hud_utility.rental_subsidy_types.values + ['Total'],
      first_column: 'B',
    ...
    sub_pops.values.each.with_index do |population_clause, col_index|
      ...
      HmisStructure::Base.current_hud_utility.rental_subsidy_types.keys.each.with_index(2) do |code, row_index|
      ...
      sheet.update_cell_members(
        ...
        members: scope.where(a_t[:exit_destination_subsidy_type].in(HmisStructure::Base.current_hud_utility.rental_subsidy_types.keys)),
      )
    end
  end
end
