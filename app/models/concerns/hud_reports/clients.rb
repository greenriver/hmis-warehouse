...
module HudReports::Clients
  ...
  included do
    ...
    private def annual_assessment_expected?(hoh_enrollment:, enrollment:, report_end_date: Date.current)
      ...
      years_in_project -= 1 if start_for_annual + years_in_project.years > report_end_date

      # If the HoH’s number of years in the project is 0, the household is not yet required to have an annual
      # assessment.
      return false if years_in_project.zero?

      # If the HoH’s number of years in the project is more than 0, add that number to the year of the HoH’s
      ...
      # household after the annual assessment may have been collected), that person is not yet required to have an
      # annual assessment.
      return false if enrollment.first_date_in_program > annual_due

      # An annual assessment is considered due for the client if they are in the project at the time annual due date
      annual_due <= [enrollment.last_date_in_program, report_end_date].compact.min
    end
