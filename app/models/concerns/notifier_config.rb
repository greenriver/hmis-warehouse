module NotifierConfig
  ...
  included do
    ...
    def setup_notifier(username)
      @notifier_config = Rails.application.config_for(:exception_notifier)['slack']
      @send_notifications = notifier_config['webhook_url'].present? && ( Rails.env.development? || Rails.env.production? )
