  ...
  included do
    scope :cas_active, -> do
      ...
      when :active_clients
        ...
        homeless_ce_project_ids = GrdaWarehouse::Hud::Project.with_project_type(Hud.util.homeless_project_types + [14]).pluck(:id)
      ...
      when :ce_with_assessment
        enrollment_scope = GrdaWarehouse::ServiceHistoryEnrollment.entry.
          in_project_type(Hud.util.performance_reporting[:ce]).
          ongoing.
    ...
    def enrolled_in_rrh_pre_move_in(ongoing_enrollments)
      ...
      ongoing_enrollments.select do |en|
        en.project_type.in?(project_type_codes) &&
          ...
          en.rrh_sub_type != Hud.util.rrh_sub_type_sso_only # not services only
      end.any?
    end
    ...
    def enrolled_in_th(ongoing_enrollments)
      ...
      project_type_codes = Hud.util.residential_project_type_numbers_by_code[:th]
      ongoing_enrollments.select do |en|
        en.project_type.in?(project_type_codes)
      end.any?
    end
    ...
    def enrolled_in_sh(ongoing_enrollments)
      ...
      project_type_codes = Hud.util.residential_project_type_numbers_by_code[:sh]
      ongoing_enrollments.select do |en|
        en.project_type.in?(project_type_codes)
      end.any?
    end
    ...
    def enrolled_in_so(ongoing_enrollments)
      ...
      project_type_codes = Hud.util.residential_project_type_numbers_by_code[:so]
      ongoing_enrollments.select do |en|
        en.project_type.in?(project_type_codes)
      end.any?
    end
    ...
    def enrolled_in_es(ongoing_enrollments)
      ...
      project_type_codes = Hud.util.residential_project_type_numbers_by_code[:es]
      ongoing_enrollments.select do |en|
        en.project_type.in?(project_type_codes)
      end.any?
    end
