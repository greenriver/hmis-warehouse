...
class ExitClientReport
  ...
  def clients
    @clients ||= begin
      ...
      client_batch = client_batch.where(destination: ::HudUtility2026.permanent_destinations) if @filter.ph
  ...
  def buckets
    @buckets ||= begin
      ...
      clients.each do |row|
        ...
        destination = 99 unless HudUtility2026.valid_destinations.key?(row[:destination])
