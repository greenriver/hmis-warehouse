...
class ExitClientReport
  ...
  def clients
    @clients ||= begin
      ...
      client_batch = client_batch.where(destination: ::HudUtilityCurrent.permanent_destinations) if @filter.ph
  ...
  def buckets
    @buckets ||= begin
      ...
      clients.each do |row|
        ...
        destination = 99 unless HudUtilityCurrent.valid_destinations.key?(row[:destination])
