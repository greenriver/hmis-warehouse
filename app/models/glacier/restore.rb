...
module Glacier
  class Restore < AwsService
    ...
    def initialize(archive_id:, download_path: nil, processing_cmd: nil)
      ...
      raise 'You need either a download path or a command to process the stream of data' if no_output || two_outputs
    end
    ...
    def run!
      ...
      if download_path.present?
        download_to_file!
      elsif processing_cmd.present?
        stream_to_command!
      end
    end

    def clear_existing_job!
      archive.update_attribute(:job_id, nil)
    end
    ...
    def initiate_job!
      ...
      archive.update_attribute(:job_id, resp.job_id)
    ...
    def download_to_file!
      Rails.logger.info "Downloading the archive to #{download_path}"

      self.processing_cmd = "cat > #{download_path}"
      stream_to_command!
    end

    def stream_to_command!
      Rails.logger.info "Streaming the archive to #{processing_cmd}"

      chunk = 32.megabytes # for testing -> 200.kilobytes
      start_byte = 0

      next_end_byte = -> {
        if (start_byte + chunk) >= archive.size_in_bytes
      ...
      Open3.popen2(processing_cmd) do |cmd_stdin, cmd_stdout, _wait_thread|
        while start_byte != archive.size_in_bytes
