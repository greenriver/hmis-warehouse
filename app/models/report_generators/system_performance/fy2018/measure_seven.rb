   ...
   private
    ...
    def calculate_7a_1
      ...
      @support[:sevena1_c3][:support] = {
        headers: ['Client ID', 'Destination'],
        counts: temporary_leavers.map{|id, destination| [id, HudUtility.destination(destination)]},
      }
    ...
    def calculate_7b_1
      ...
      universe.each do |id|
        ...
        exit_data = destination_scope.joins(:enrollment).
          order(date: :desc).
          limit(1).
          pluck(:destination, :project_type, e_t[:MoveInDate].to_sql).map do |destination, project_type, move_in_date|
