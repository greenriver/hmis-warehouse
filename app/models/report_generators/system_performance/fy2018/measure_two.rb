...
module ReportGenerators::SystemPerformance::Fy2018
  class MeasureTwo < Base
    ...
    PERMANENT_DESTINATIONS = [3, 10, 11, 19, 20, 21, 22, 23, 26, 28, 31].freeze
    ...
    def calculate # rubocop:disable Metrics/AbcSize, Metrics/CyclomaticComplexity
      if start_report(Reports::SystemPerformance::Fy2018::MeasureTwo.first)
        ...
        project_exits_universe.each do |p_exit|
          ...
          next unless PERMANENT_DESTINATIONS.include? p_exit[:destination]

          project_exits_to_ph[p_exit[:client_id]] = p_exit unless project_exits_to_ph[p_exit[:client_id]].present?
        end

        project_exits_to_ph.each do |_id, p_exit|
        ...
        project_exits_to_ph.each do |_id, p_exit|
          ...
          client_entries_all.each do |entry|
            client_entries[entry[:first_date_in_program]] = [] if client_entries[entry[:first_date_in_program]].nil?
          ...
          client_entries.each do |day, i_project_types|
            ...
            # If the next destination is PH, it must be > 14 days since exit to count
              ...
              if (day.to_date - ph_check_date.to_date).to_i < 14
                ...
                break if next_end_date.nil? # rubocop:disable Metrics/BlockNesting

                ph_check_date = next_end_date
              else
                case (day.to_date - p_exit[:last_date_in_program].to_date).to_i # rubocop:disable Metrics/BlockNesting
                when (0..180)
                  case p_exit[:project_type].to_i # rubocop:disable Metrics/BlockNesting
                  when *SO
                    project_exit_counts[:c_0_180_days][:so][:counts] << day_count
            ...
            # If the next destination is TH, it must be > 14 days since exit to count
              ...
              if day_count > 14
                case day_count # rubocop:disable Metrics/BlockNesting
                when (0..180)
                  case p_exit[:project_type].to_i # rubocop:disable Metrics/BlockNesting
                  when *SO
                    project_exit_counts[:c_0_180_days][:so][:counts] << day_count
