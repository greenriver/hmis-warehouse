...
class Health::ExpiringItemReport
  ...
  def expired_pctp_forms
    patient_source.joins(:recent_pctp_form).merge(
      pctp_forms_source.expired.
        where.not(patient_id_column.in(active_pctp_forms.select(patient_id_column))),
    ).distinct.
      preload(:recent_pctp_form)
  end

  def expiring_pctp_forms
    patient_id_column = pctp_forms_source.arel_table[:patient_id]
    patient_source.joins(:recent_pctp_form).merge(
      pctp_forms_source.expiring_soon.
        where.not(patient_id_column.in(recently_completed_pctp_forms.select(patient_id_column))),
    ).distinct.
      preload(:recent_pctp_form)
  end
  ...
  private def patient_id_column
    pctp_forms_source.arel_table[:patient_id]
  end
