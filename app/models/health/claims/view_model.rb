...
module Health::Claims
  class ViewModel
    ...
    def load_key_metric_values
      ...
      implementation_ave_ed_visits = @patient.ed_nyu_severities.sum(:implementation_visits).to_f / implementation_months rescue 0 # rubocop:disable Style/RescueModifier
      baseline_ave_ed_visits = @patient.ed_nyu_severities.sum(:baseline_visits).to_f / baseline_months rescue 0 # rubocop:disable Style/RescueModifier
      implementation_ip_admit_ave = @patient_roster.implementation_admits.to_f / implementation_months rescue 0 # rubocop:disable Style/RescueModifier
      baseline_ip_admit_ave = @patient_roster.baseline_admits.to_f / baseline_months rescue 0 # rubocop:disable Style/RescueModifier
      implementation = [
    ...
    def load_cost_values
      ...
      [patient, sdh].each do |arry|
        arry.push((arry[0] / arry[1].to_f).round) rescue 0 # rubocop:disable Style/RescueModifier
      end
      [patient, sdh]
    end

    def display_value(key, value, formatted_value)
      if value.present?
        key == :variance ? "#{value}%" : formatted_value
      else
        'N/A'
      end
    end
    ...
    def baseline_variance(implementation, baseline)
      if implementation && baseline && baseline != 0
        (((implementation - baseline) / baseline.to_f) * 100).round rescue 'N/A' # rubocop:disable Style/RescueModifier
      else
        'N/A'
      end
    end
