...
module Bo
  class Soap
    ...
    def data_from url, xml
      ...
      if response.response_code == 200 && response.body_str.present?
    ...
    def request url, xml
      url = url + "&authType=secEnterprise&locale=en_US&timeout=60&ConvertAnyType=false"
      result = Curl.post(url, xml) do |curl|
