...
class ClientHistory
  ...
  def dates
    @dates.transform_values do |data|
      data.select { |en| ::HudUtilityCurrent.residential_project_type_ids.include?(en[:project_type]) }
    end
    @dates
  end
  ...
  def organization_counts
    dates.values.flatten.group_by { |en| HudUtilityCurrent.project_type en[:organization_name] }.transform_values(&:count)
  end

  def project_type_counts
    dates.values.flatten.group_by { |en| HudUtilityCurrent.project_type en[:project_type] }.transform_values(&:count)
  end
