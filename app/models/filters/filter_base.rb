...
module Filters
  class FilterBase < ::ModelForm
    ...
    attribute :default_project_type_codes, Array, default: Hud.util.homeless_project_type_codes
    ...
    def update(filters) # rubocop:disable Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity, Metrics/AbcSize
      ...
      self.races = filters.dig(:races)&.select { |race| Hud.util.races(multi_racial: true).keys.include?(race) }.presence || races
      ...
      self.ethnicities = filters.dig(:ethnicities)&.select { |ethnicity| Hud.util.ethnicities.keys.include?(ethnicity.to_s.to_sym) }.presence&.map(&:to_sym) || ethnicities
      self.race_ethnicity_combinations = filters.dig(:race_ethnicity_combinations)&.select { |value| Hud.util.race_ethnicity_combinations.keys.include?(value.to_sym) }.presence&.map(&:to_sym) || race_ethnicity_combinations
    ...
    def project_type_options_for_select(id_limit: [])
      options = Hud.util.project_types.invert
    ...
    def project_type_code_options_for_select
      Hud.util.project_type_group_titles.select { |k, _| k.in?(default_project_type_codes) }.freeze.invert
    end
    ...
    def available_project_types
      Hud.util.project_type_group_titles.invert
    end

    def available_residential_project_types
      Hud.util.residential_type_titles.invert
    end

    def available_homeless_project_types
      Hud.util.homeless_type_titles.invert
    end
    ...
    def available_times_homeless_in_last_three_years
      ::Hud.util.times_homeless_options
    end
    ...
    def project_type_ids
      ids = Hud.util.performance_reporting.values_at(
    ...
    def selected_project_type_names
      Hud.util.residential_type_titles.values_at(*project_type_codes.reject(&:blank?).map(&:to_sym))
    end
    ...
    def default_project_type_numbers
      Hud.util.project_types_with_inventory
    end
    ...
    def chosen_excluded_project_type_numbers
      ...
      excluded_project_type_numbers.map { |number| Hud.util.project_type(number) }
    end
    ...
    def chosen_funding_sources
      ...
      funder_ids.map { |code| "#{Hud.util.funding_source(code&.to_i)} (#{code})" }
    end
    ...
    def chosen_veteran_statuses
      veteran_statuses.map do |veteran_status|
        Hud.util.veteran_status(veteran_status)
      end
    end

    def chosen_project_types
      project_type_ids.map do |type|
        Hud.util.project_type(type)
      end.uniq
    end

    def chosen_project_types_only_homeless?
      project_type_ids.sort == Hud.util.homeless_project_types.sort
    end
    ...
    def available_prior_living_situations(grouped: false)
      if grouped
        {
          'Homeless' => Hud.util.homeless_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
          end.to_h,
          'Institutional' => Hud.util.institutional_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
          end.to_h,
          'Temporary' => Hud.util.temporary_housing_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
          end.to_h,
          'Permanent' => Hud.util.permanent_housing_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
          end.to_h,
          'Other' => Hud.util.other_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
        ...
        Hud.util.living_situations.map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
    ...
    def available_destinations(grouped: false)
      return Hud.util.valid_destinations.invert unless grouped

      {
        'Homeless' => Hud.util.homeless_situation_options(as: :destination).map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
        end.to_h,
        'Institutional' => Hud.util.institutional_situation_options(as: :destination).map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
        end.to_h,
        'Temporary' => Hud.util.temporary_destination_options.map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
        end.to_h,
        'Permanent' => Hud.util.permanent_destination_options.map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
        end.to_h,
        'Other' => Hud.util.other_situation_options(as: :destination).map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
    ...
    def available_disabilities
      @available_disabilities ||= {}.tap do |disabilities|
        Hud.util.disability_types.each do |id, title|
    ...
    def available_indefinite_disabilities
      Hud.util.no_yes_reasons_for_missing_data_options.invert
    end
