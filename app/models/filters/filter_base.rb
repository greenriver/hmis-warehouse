...
module Filters
  class FilterBase < ::ModelForm
    ...
    def update(filters) # rubocop:disable Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity, Metrics/AbcSize
      ...
      self.races = filters.dig(:races)&.select { |race| HudHelper.util.races(multi_racial: true).keys.include?(race) }.presence || races
      ...
      self.ethnicities = filters.dig(:ethnicities)&.select { |ethnicity| HudHelper.util.ethnicities.keys.include?(ethnicity.to_s.to_sym) }.presence&.map(&:to_sym) || ethnicities
      self.race_ethnicity_combinations = filters.dig(:race_ethnicity_combinations)&.select { |value| HudHelper.util.race_ethnicity_combinations.keys.include?(value.to_sym) }.presence&.map(&:to_sym) || race_ethnicity_combinations
    ...
    def project_type_options_for_select(id_limit: [])
      options = HudHelper.util.project_types.invert
    ...
    def available_project_types
      HudHelper.util.project_type_group_titles.invert
    end

    def available_residential_project_types
      HudHelper.util.residential_type_titles.invert
    end

    def available_homeless_project_types
      HudHelper.util.homeless_type_titles.invert
    end
    ...
    def available_times_homeless_in_last_three_years
      ::HudHelper.util.times_homeless_options
    end
    ...
    def project_type_ids
      ids = HudHelper.util.performance_reporting.values_at(
    ...
    def selected_project_type_names
      HudHelper.util.residential_type_titles.values_at(*project_type_codes.reject(&:blank?).map(&:to_sym))
    end
    ...
    def default_project_type_numbers
      HudHelper.util.project_types_with_inventory
    end
    ...
    def chosen_races
      races.map do |race|
        HudHelper.util.race(race, multi_racial: true)
      end
    end

    def chosen_race_ethnicity_combinations
      race_ethnicity_combinations.map do |combination|
        HudHelper.util.race_ethnicity_combination(combination)
      end
    end

    def chosen_ethnicities
      ethnicities.map do |ethnicity|
        HudHelper.util.ethnicity(ethnicity)
      end
    end

    def chosen_genders
      genders.map do |gender|
        HudHelper.util.gender(gender)
      end
    end
    ...
    def chosen_excluded_project_type_numbers
      ...
      excluded_project_type_numbers.map { |number| HudHelper.util.project_type(number) }
    end
    ...
    def chosen_funding_sources
      ...
      funder_ids.map { |code| "#{HudHelper.util.funding_source(code&.to_i)} (#{code})" }
    end
    ...
    def chosen_veteran_statuses
      veteran_statuses.map do |veteran_status|
        HudHelper.util.veteran_status(veteran_status)
      end
    end

    def chosen_project_types
      project_type_ids.map do |type|
        HudHelper.util.project_type(type)
      end.uniq
    end

    def chosen_project_types_only_homeless?
      project_type_ids.sort == HudHelper.util.homeless_project_types.sort
    end
    ...
    def available_prior_living_situations(grouped: false)
      if grouped
        {
          'Homeless' => HudHelper.util.homeless_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
          end.to_h,
          'Institutional' => HudHelper.util.institutional_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
          end.to_h,
          'Temporary' => HudHelper.util.temporary_housing_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
          end.to_h,
          'Permanent' => HudHelper.util.permanent_housing_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
          end.to_h,
          'Other' => HudHelper.util.other_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
        ...
        HudHelper.util.living_situations.map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
    ...
    def available_destinations(grouped: false)
      return HudHelper.util.valid_destinations.invert unless grouped

      {
        'Homeless' => HudHelper.util.homeless_situation_options(as: :destination).map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
        end.to_h,
        'Institutional' => HudHelper.util.institutional_situation_options(as: :destination).map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
        end.to_h,
        'Temporary' => HudHelper.util.temporary_destination_options.map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
        end.to_h,
        'Permanent' => HudHelper.util.permanent_destination_options.map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
        end.to_h,
        'Other' => HudHelper.util.other_situation_options(as: :destination).map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
    ...
    def available_disabilities
      @available_disabilities ||= {}.tap do |disabilities|
        HudHelper.util.disability_types.each do |id, title|
    ...
    def available_indefinite_disabilities
      HudHelper.util.no_yes_reasons_for_missing_data_options.invert
    end
