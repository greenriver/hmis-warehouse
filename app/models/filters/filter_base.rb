...
module Filters
  class FilterBase < ::ModelForm
    ...
    attribute :default_project_type_codes, Array, default: HudUtilityCurrent.homeless_project_type_codes
    ...
    def update(filters) # rubocop:disable Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity, Metrics/AbcSize
      ...
      self.default_project_type_codes = normalize_input_array(filters[:default_project_type_codes], &:to_s) || default_project_type_codes if filters.key?(:default_project_type_codes)
      if filters.key?(:project_type_codes)
        self.project_type_codes = normalize_input_array(filters[:project_type_codes], &:to_s) || project_type_codes
      elsif filters.key?(:project_type_numbers)
        self.project_type_codes = []
      else
        self.project_type_codes = project_type_codes
      end
      self.project_type_numbers = normalize_input_array(filters[:project_type_numbers]) || project_type_numbers
      self.data_source_ids = normalize_input_array(filters[:data_source_ids]) || data_source_ids
      self.organization_ids = normalize_input_array(filters[:organization_ids]) || organization_ids
      self.project_ids = normalize_input_array(filters[:project_ids]) || project_ids
      self.funder_ids = normalize_input_array(filters[:funder_ids]) || funder_ids
      self.funder_others = normalize_input_array(filters[:funder_others], &:to_s) || funder_others
      self.veteran_statuses = normalize_input_array(filters[:veteran_statuses]) || veteran_statuses
      ...
      self.races = filters.dig(:races)&.select { |race| HudUtilityCurrent.races(multi_racial: true).keys.include?(race) }.presence || races
      ...
      self.length_of_times = normalize_input_array(filters[:length_of_times], &:to_sym) || length_of_times
      self.cohort_ids = normalize_input_array(filters[:cohort_ids]) || cohort_ids
      self.secondary_cohort_ids = normalize_input_array(filters[:secondary_cohort_ids]) || secondary_cohort_ids
      ...
      self.disabilities = normalize_input_array(filters[:disabilities]) || disabilities
      ...
      self.indefinite_disabilities = normalize_input_array(filters[:indefinite_disabilities]) || indefinite_disabilities
      self.dv_status = normalize_input_array(filters[:dv_status]) || dv_status
      self.currently_fleeing = normalize_input_array(filters[:currently_fleeing]) || currently_fleeing
      ...
      self.required_files = normalize_input_array(filters[:required_files]) || required_files
      self.optional_files = normalize_input_array(filters[:optional_files]) || optional_files
      ...
      self.ethnicities = filters.dig(:ethnicities)&.select { |ethnicity| HudUtilityCurrent.ethnicities.keys.include?(ethnicity.to_s.to_sym) }.presence&.map(&:to_sym) || ethnicities
      self.race_ethnicity_combinations = filters.dig(:race_ethnicity_combinations)&.select { |value| HudUtilityCurrent.race_ethnicity_combinations.keys.include?(value.to_sym) }.presence&.map(&:to_sym) || race_ethnicity_combinations
    ...
    private def normalize_input_array(dirty, &normalizer)
      normalizer ||= proc(&:to_i) # default transformation is to_i
      Array.wrap(dirty).compact_blank.map(&normalizer).sort.uniq.presence
    end
    ...
    DEFAULT_LABELS = {
      ...
      excluded_project_type_numbers: 'Excluded Project Types',
    }.freeze
    ...
    def reject_excluded_project_ids(project_ids)
      ...
      project_ids = project_ids.reject { |id| excluded_project_ids.include?(id) }
    ...
    def project_type_options_for_select(id_limit: [])
      options = HudUtilityCurrent.project_types.invert
    ...
    def project_type_code_options_for_select
      HudUtilityCurrent.project_type_group_titles.select { |k, _| k.in?(default_project_type_codes) }.freeze.invert
    end
    ...
    def available_project_types
      HudUtilityCurrent.project_type_group_titles.invert
    end

    def available_residential_project_types
      HudUtilityCurrent.residential_type_titles.invert
    end

    def available_homeless_project_types
      HudUtilityCurrent.homeless_type_titles.invert
    end
    ...
    def available_times_homeless_in_last_three_years
      ::HudUtilityCurrent.times_homeless_options
    end
    ...
    def project_type_ids
      ids = HudUtilityCurrent.performance_reporting.values_at(
    ...
    def selected_project_type_names
      HudUtilityCurrent.residential_type_titles.values_at(*project_type_codes.reject(&:blank?).map(&:to_sym))
    end
    ...
    def default_project_type_numbers
      HudUtilityCurrent.project_types_with_inventory
    end
    ...
    def chosen_races
      races.map do |race|
        HudUtilityCurrent.race(race, multi_racial: true)
      end
    end

    def chosen_race_ethnicity_combinations
      race_ethnicity_combinations.map do |combination|
        HudUtilityCurrent.race_ethnicity_combination(combination)
      end
    end

    def chosen_ethnicities
      ethnicities.map do |ethnicity|
        HudUtilityCurrent.ethnicity(ethnicity)
      end
    end

    def chosen_genders
      genders.map do |gender|
        HudUtilityCurrent.gender(gender)
      end
    end
    ...
    def chosen_excluded_project_type_numbers
      ...
      excluded_project_type_numbers.map { |number| HudUtilityCurrent.project_type(number) }
    end
    ...
    def chosen_funding_sources
      ...
      funder_ids.map { |code| "#{HudUtilityCurrent.funding_source(code&.to_i)} (#{code})" }
    end
    ...
    def chosen_veteran_statuses
      veteran_statuses.map do |veteran_status|
        HudUtilityCurrent.veteran_status(veteran_status)
      end
    end

    def chosen_project_types
      project_type_ids.map do |type|
        HudUtilityCurrent.project_type(type)
      end.uniq
    end

    def chosen_project_types_only_homeless?
      project_type_ids.sort == HudUtilityCurrent.homeless_project_types.sort
    end
    ...
    def available_prior_living_situations(grouped: false)
      if grouped
        {
          'Homeless' => HudUtilityCurrent.homeless_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
          end.to_h,
          'Institutional' => HudUtilityCurrent.institutional_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
          end.to_h,
          'Temporary' => HudUtilityCurrent.temporary_housing_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
          end.to_h,
          'Permanent' => HudUtilityCurrent.permanent_housing_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
          end.to_h,
          'Other' => HudUtilityCurrent.other_situation_options(as: :prior).map do |id, title|
            [
              "#{title} (#{id})",
              id,
            ]
        ...
        HudUtilityCurrent.living_situations.map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
    ...
    def available_destinations(grouped: false)
      return HudUtilityCurrent.valid_destinations.invert unless grouped

      {
        'Homeless' => HudUtilityCurrent.homeless_situation_options(as: :destination).map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
        end.to_h,
        'Institutional' => HudUtilityCurrent.institutional_situation_options(as: :destination).map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
        end.to_h,
        'Temporary' => HudUtilityCurrent.temporary_destination_options.map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
        end.to_h,
        'Permanent' => HudUtilityCurrent.permanent_destination_options.map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
        end.to_h,
        'Other' => HudUtilityCurrent.other_situation_options(as: :destination).map do |id, title|
          [
            "#{title} (#{id})",
            id,
          ]
    ...
    def available_disabilities
      @available_disabilities ||= {}.tap do |disabilities|
        HudUtilityCurrent.disability_types.each do |id, title|
    ...
    def available_indefinite_disabilities
      HudUtilityCurrent.no_yes_reasons_for_missing_data_options.invert
    end
