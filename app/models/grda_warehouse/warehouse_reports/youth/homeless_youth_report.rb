  ...
  class HomelessYouthReport
    ...
    def d_one_b
      @d_one_b ||= get_client_ids(all_served_last_assessment.
        where(client_gender: 1)) # HudHelper.util('legacy').gender man
    end

    def d_one_c
      @d_one_c ||= get_client_ids(all_served_last_assessment.
          where(client_gender: 0)) # HudHelper.util('legacy').gender woman
    end

    def d_one_d
      @d_one_d ||= get_client_ids(all_served_last_assessment.
          where(client_gender: [2, 3])) # HudHelper.util('legacy').gender trans
    end

    def d_one_e
      @d_one_e ||= get_client_ids(all_served_last_assessment.
          where(client_gender: 4)) # HudHelper.util('legacy').gender non-binary
    end

    def d_one_f
      @d_one_f ||= get_client_ids(all_served_last_assessment.
          where(client_gender: [6, 8, 9])) # HudHelper.util('legacy').gender questioning, 8, 9
    end

    def d_one_g
      @d_one_g ||= get_client_ids(all_served_last_assessment.
          where(client_gender: 99)) # HudHelper.util('legacy').gender 99
    end
