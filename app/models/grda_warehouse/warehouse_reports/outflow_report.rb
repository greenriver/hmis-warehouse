  ...
  class OutflowReport
    ...
    def clients_to_permanent_or_neutral
      @clients_to_permanent_or_neutral ||= (exits_scope.
        where(destination: Hud.util.permanent_destinations + neutral_destinations).
        distinct.
    ...
    def clients_to_destinations
      @clients_to_destinations ||= (exits_scope.
        where(destination: Hud.util.permanent_destinations + neutral_destinations + jail_destinations + deceased_destinations).
        distinct.
        pluck(:client_id) + clients_to_stabilization).uniq
    end
    ...
    def outflow_to_housing
      ...
      hoh_to_stabilization = housed_scope.
        heads_of_households.
        where(project_type: Hud.util.project_types_with_move_in_dates). # PSH or RRH
      ...
      enrollments_for(key).values.each do |enrollments|
        enrollments.each do |enrollment|
          ...
          row += [
            ...
            Hud.util.destination(enrollment.destination),
          ]
          rows << row
        end
      end
