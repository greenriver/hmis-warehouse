...
module GrdaWarehouse::WarehouseReports::Project::DataQuality
  class VersionTwo < Base
    ...
    def add_enrolled_length_of_stay
      ...
      totals[:counts][:average] = (totals[:counts][:total_days].to_f / (self.end - start).to_i).round
    ...
    def add_missing_enrollment client_id:, enrollment:, counts:
      ...
      counts['missing_residence_prior'] << columns_for_residence_prior_support(enrollment) if missing?(enrollment[:residence_prior]) && adult?(enrollment[:age]) && enrollment[:head_of_household]
    ...
    def add_refused_enrollment client_id:, enrollment:, counts:
      ...
      counts['refused_residence_prior'] << columns_for_residence_prior_support(enrollment) if refused?(enrollment[:residence_prior]) && adult?(enrollment[:age]) && enrollment[:head_of_household]
    ...
    def add_unknown_enrollment client_id:, enrollment:, counts:
      ...
      counts['unknown_residence_prior'] << columns_for_residence_prior_support(enrollment) if unknown?(enrollment[:residence_prior]) && adult?(enrollment[:age]) && enrollment[:head_of_household]
    ...
    def add_missing_demo client:, counts:
      ...
      counts['missing_ssn'] << columns_for_ssn_support(client) if alternate_clients.map { |m| m[:ssn] }.all?(&:blank?) || alternate_clients.map { |m| missing?(m[:ssn_data_quality]) }.all?
      counts['missing_dob'] << columns_for_dob_support(client) if alternate_clients.map { |m| m[:dob] }.all?(&:blank?) || alternate_clients.map { |m| missing?(m[:dob_data_quality]) }.all?
    ...
    def add_refused_demo client:, counts:
      ...
      counts['refused_ssn'] << columns_for_ssn_support(client) if alternate_clients.map { |m| refused?(m[:ssn_data_quality]) }.all?
      counts['refused_dob'] << columns_for_dob_support(client) if alternate_clients.map { |m| refused?(m[:dob_data_quality]) }.all?
    ...
    def add_unknown_demo client:, counts:
      ...
      counts['unknown_ssn'] << columns_for_ssn_support(client) if alternate_clients.map { |m| unknown?(m[:ssn_data_quality]) }.all?
      counts['unknown_dob'] << columns_for_dob_support(client) if alternate_clients.map { |m| unknown?(m[:dob_data_quality]) }.all?
    ...
    def calculate_missing_universal_elements
      ...
      clients.each do |client|
        ...
        missing_ssn << client[:destination_id] if client[:ssn].blank? || missing?(client[:ssn_data_quality])
        missing_dob << client[:destination_id] if client[:dob].blank? || missing?(client[:dob_data_quality])
        ...
        # If we have no race info, whatsoever
        missing_race << client[:destination_id] if missing?(client[:race_none]) && missing?(client[:am_ind_ak_native]) && missing?(client[:asian]) && missing?(client[:black_af_american]) && missing?(client[:native_hi_other_pacific]) && missing?(client[:white])
        ...
        refused_ssn << client[:destination_id] if client[:ssn].blank? || refused?(client[:ssn_data_quality])
        refused_dob << client[:destination_id] if client[:dob].blank? || refused?(client[:dob_data_quality])
        ...
        refused_gender << client[:destination_id] if client[:gender].blank? || refused?(client[:gender])
      end
    ...
    def meets_data_quality_benchmark
      ...
      meets_dq_benchmark = begin
                             report.with_indifferent_access.values_at(*percentages).max < MISSING_THRESHOLD
                           rescue StandardError
                             false
                           end
      add_answers({
    ...
    def destination_ph
      ...
      ph_destinations_percentage = begin
                                     (ph_destinations.size.to_f / leavers.size * 100).round(2)
                                   rescue StandardError
                                     0
                                   end
      answers = {
