    ...
    def clients_with_no_income
      @clients_with_no_income ||= begin
        ...
        data.each do |row|
          ...
          if row[:income_at_later_date_response] == 0
            counts[:earned] << row[:client_id]
          elsif row[:income_at_later_date_response] == 1
            value = row[:income_at_later_date_earned]
            counts[:earned] << row[:client_id] if value.nil? || value.zero?
          elsif row[:income_at_later_date_response].nil?
            if row[:income_at_entry_response] == 0
              counts[:earned] << row[:client_id]
            elsif row[:income_at_entry_response] == 1
              value = row[:income_at_entry_earned]
              counts[:earned] << row[:client_id] if value.nil? || value.zero?
            end
          end

          if row[:income_at_later_date_response] == 0
            counts[:overall] << row[:client_id]
          elsif row[:income_at_later_date_response] == 1
            value = row[:income_at_later_date_overall]
            counts[:overall] << row[:client_id] if value.nil? || value.zero?
          elsif row[:income_at_later_date_response].nil?
            if row[:income_at_entry_response] == 0
              counts[:overall] << row[:client_id]
            elsif row[:income_at_entry_response] == 1
              value = row[:income_at_entry_overall]
              counts[:overall] << row[:client_id] if value.nil? || value.zero?
            end
          end

          if row[:income_at_later_date_response] == 0
            counts[:non_employment_cash] << row[:client_id]
          elsif row[:income_at_later_date_response] == 1
            value = row[:income_at_later_date_non_employment_cash]
            counts[:non_employment_cash] << row[:client_id] if value.nil? || value.zero?
          elsif row[:income_at_later_date_response].nil?
            if row[:income_at_entry_response] == 0
              counts[:non_employment_cash] << row[:client_id]
            elsif row[:income_at_entry_response] == 1
              value = row[:income_at_entry_non_employment_cash]
              counts[:non_employment_cash] << row[:client_id] if value.nil? || value.zero?
            end
          end
        end
