...
module GrdaWarehouse::WarehouseReports::Exports
  class AdHoc < GrdaWarehouseBase
    ...
    private def race_for_client(client)
      ...
      fields.map { |f| ::HudUtilityCurrent.race f }.join ', '
    end
        ...
        client_scope.distinct.in_batches(of: 100) do |batch|
          ...
          batch.find_each do |client|
            rows << [
              ...
              HudUtilityCurrent.project_type(report_calculator.enrollment_for_client(client)&.project&.project_type),
              HudUtilityCurrent.destination(report_calculator.exit_for_client(client)&.Destination),
              HudUtilityCurrent.destination(report_calculator.most_recent_exit_with_destination_for_client(client)&.Destination),
