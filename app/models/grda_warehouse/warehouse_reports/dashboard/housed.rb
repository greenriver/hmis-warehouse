    ...
    def run!
      ...
      all_exits = exits_from_homelessness.
        ...
        pluck(*columns).map do |date, destination, client_id|
          destination = 99 unless ::HudHelper.util.valid_destinations.keys.include?(destination)
      ...
      @ph_clients = all_exits.select { |m| ::HudHelper.util.permanent_destinations.include?(m[:destination]) }.map { |m| m[:client_id] }.uniq

      @buckets = {}

      all_destinations.each do |destination|
        label = ::HudHelper.util.destination(destination).to_s
        label = ::HudHelper.util.destination(99) if label.is_a? Numeric
        @buckets[destination] ||= {
          source_data: all_date_buckets.deep_dup,
          label: label.truncate(45),
          backgroundColor: colorize(label),
          ph: ::HudHelper.util.permanent_destinations.include?(destination),
        }
      end
