...
module GrdaWarehouse
  class Cohort < GrdaWarehouseBase
    ...
    private def destination_from_homelessness(client)
      client.permanent_source_exits_from_homelessness.
        ...
        pluck(:ExitDate, :Destination).map do |exit_date, destination|
          "<span class='hidden'>#{exit_date.to_fs(:db)}</span>#{exit_date} to #{HudUtilityCurrent.destination(destination)}"
        end.join('; ')
    end
    ...
    private def calculated_most_recent_self_report_months_homeless(client)
      ...
      "#{HudUtilityCurrent.months_homeless_past_three_years(item.months_homeless_past_three_years)} on #{item.entry_date} at #{item.project.name}"
    end
    ...
    private def calculated_most_recent_disabling_condition(client)
      ...
      "#{HudUtilityCurrent.no_yes_reasons_for_missing_data(item.disabling_condition)} on #{item.entry_date} at #{item.project.name}"
    end
