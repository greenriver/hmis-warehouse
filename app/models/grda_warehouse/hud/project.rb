...
module GrdaWarehouse::Hud
  class Project < Base
    ...
    scope :hud_residential, -> do
      _residential_for_project_type_ids(HudUtilityCurrent.residential_project_type_ids)
    end
    ...
    scope :homeless, -> do
      where(project_type: HudUtilityCurrent.homeless_project_types)
    end
    ...
    scope :homeless_sheltered, -> do
      where(project_type: HudUtilityCurrent.homeless_sheltered_project_types)
    end

    scope :homeless_unsheltered, -> do
      where(project_type: HudUtilityCurrent.homeless_unsheltered_project_types)
    end
    ...
    def rrh?
      project_type_to_use.in?(HudUtilityCurrent.performance_reporting[:rrh])
    end
    ...
    def rrh_sso_sub_type_code
      HudUtilityCurrent.rrh_sub_type('RRH: Services Only', true)
    end

    def psh?
      project_type_to_use.in?(HudUtilityCurrent.performance_reporting[:psh])
    end

    def homeless?
      project_type_to_use.in?(HudUtilityCurrent.homeless_project_type_numbers)
    end
    ...
    def name(user = nil, include_project_type: false, ignore_confidential_status: false)
      ...
      project_name += " (#{HudUtilityCurrent.brief_project_type_with_sub_type(project_type, rrh_sub_type)})" if include_project_type && project_type.present?
        ...
        connection.select_all(projects.to_sql).each do |project|
          ...
          headers.each do |h|
            ...
            when :current_continuum_project
              ::HudUtilityCurrent.ad_hoc_yes_no project[h.to_s].presence&.to_i
            when :fed_partner_program
              ::HudUtilityCurrent.funding_source project[h.to_s].presence&.to_i
            else
              project[h.to_s]
            end
            row << value
    ...
    def pay_for_success?
      return false unless HudUtilityCurrent.performance_reporting[:other].include?(project_type)

      funders.map(&:pay_for_success?).any?
    end

    def human_readable_project_type
      HudUtilityCurrent.project_type(project_type_to_use)
    end
