...
module GrdaWarehouse::Hud
  class Client < Base
    ...
    scope :entered_in_range, ->(range) do
      s = range.first
      e = range.last
      exclude = range.exclude_end? # the exclusion bit's a little pedantic...
    ...
    def image(_cache_for = 10.minutes)
      ...
      source_api_ids.detect do |api_id|
        ...
        api ||= begin
                  EtoApi::Base.new(api_connection: api_key).tap { |api| api.connect }
    ...
    def fake_client_image_data
      ...
      logger.debug "Client#image id:#{id} faked #{self.PersonalID} #{available.count} #{available[image_id]}"
    ...
    def update_via_api
      ...
      Importing::RunEtoApiUpdateForClientJob.perform_later(destination_id: id, client_ids: client_ids.uniq) if client_ids.any?
    end
    ...
    def invalidate_service_history
      processed_service_history.destroy if processed_service_history.present?
    end
    ...
    def self.text_search(text, client_scope:)
      ...
      client_ids << text if numeric && destination.where(id: text).exists?
      where(id: client_ids)
    end
    ...
    def length_of_episodes start_date:, end_date:, residential_enrollments: nil, chronic_enrollments: nil
      ...
      chronic_enrollments.drop(1).map do |enrollment|
        next unless new_episode?(enrollments: residential_enrollments, enrollment: enrollment)
    ...
    def affiliated_residential_projects enrollment
      ...
      begin
        @residential_affiliations[[enrollment[:ProjectID], enrollment[:data_source_id]]].map(&:last)
      rescue StandardError
        []
      end
    end
    ...
    def affiliated_projects enrollment
      ...
      begin
        @project_affiliations[[enrollment[:ProjectID], enrollment[:data_source_id]]].map(&:last)
      rescue StandardError
        []
      end
    end
    ...
    def affiliated_projects_str_for_enrollment enrollment
      ...
      "Affiliated with #{project_names.to_sentence}" if project_names.any?
    end
    ...
    def program_tooltip_data_for_enrollment enrollment
      ...
      if affiliated_projects_str.present? || residential_projects_str.present?
        ...
        { toggle: :tooltip, title: title.to_s }
      else
        {}
      end
    end
