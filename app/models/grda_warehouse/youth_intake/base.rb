...
module GrdaWarehouse::YouthIntake
  class Base < GrdaWarehouseBase
    ...
    private def transitioning_to_housing?
      stably_housed? && client.current_youth_situation(on_date: engagement_date) != :housed
    end

    private def transitioning_to_at_risk?
      at_risk_of_homelessness? && client.current_youth_situation(on_date: engagement_date).in?([nil, :housed])
    end
