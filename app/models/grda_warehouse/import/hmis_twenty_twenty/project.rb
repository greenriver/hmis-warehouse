...
module GrdaWarehouse::Import::HmisTwentyTwenty
  class Project < GrdaWarehouse::Hud::Project
    ...
    def self.load_from_csv(file_path:, data_source_id:)
      existing_projects = with_deleted.where(data_source_id: data_source_id).pluck(hud_key, :DateUpdated, :ProjectType, :id).map do |key, updated_at, project_type, id|
      ...
      [].tap do |m|
        CSV.read(
          ...
          extra = {
            ...
            existing: OpenStruct.new(existing_projects[row[hud_key.to_s]]),
          }
