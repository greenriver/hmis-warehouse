...
class GrdaWarehouse::ServiceHistoryEnrollment < GrdaWarehouseBase
  ...
  scope :permanent_housing, -> do
    project_types = Hud.util.residential_project_type_numbers_by_code.values_at(:ph).flatten
    in_project_type(project_types)
  end

  scope :homeless_sheltered, -> do
    in_project_type(Hud.util.homeless_sheltered_project_types)
  end
  scope :homeless_unsheltered, -> do
    in_project_type(Hud.util.homeless_unsheltered_project_types)
  end
  ...
  scope :hud_homeless, ->(chronic_types_only: true) do # rubocop:disable Lint/UnusedBlockArgument
    hud_project_type(Hud.util.chronic_project_types)
  end
  ...
  scope :currently_homeless, ->(date: Date.current, chronic_types_only: false) do
    if chronic_types_only # literally homeless
      residential_project_types = Hud.util.residential_project_type_numbers_by_code[:ph] + Hud.util.residential_project_type_numbers_by_code[:th]
    else
      residential_project_types = Hud.util.residential_project_type_numbers_by_code[:ph]
    end
  ...
  def service_type
    ::Hud.util.project_type(project_type)
  end

  def service_type_brief
    ::Hud.util.project_type_brief(project_type)
  end
  ...
  def so?
    self[self.class.project_type_column] == Hud.util.project_type_number('SO')
  end

  def ce?
    self[self.class.project_type_column] == Hud.util.project_type_number('CE')
  end

  def homeless?
    self[self.class.project_type_column].in?(Hud.util.homeless_project_types)
  end
