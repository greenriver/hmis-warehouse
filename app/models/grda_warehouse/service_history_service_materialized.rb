...
class GrdaWarehouse::ServiceHistoryServiceMaterialized < GrdaWarehouseBase
  ...
  def self.remove_view_sql
    ...
    self.connection.add_index :service_history_services_materialized, [:client_id, :homeless], name: :index_shsm_homeless
    self.connection.add_index :service_history_services_materialized, [:client_id, :literally_homeless], name: :index_shsm_literally_homeless
  end
end