...
module GrdaWarehouse::Tasks
  class ChronicallyHomeless
    include TsqlImport
    include ArelHelper
    RESIDENTIAL_NON_HOMELESS_PROJECT_TYPE = HudUtilityCurrent.residential_project_type_ids - HudUtilityCurrent.chronic_project_types
    SO = 4
    ...
    def initialize(
      date: Date.current,
      count_so_as_full_month: true, # rubocop:disable Lint/UnusedMethodArgument
      dry_run: false,
      client_ids: nil,
      debug: false,
      sanity_check: false
    ...
    def load_active_clients
      ...
      dirty = GrdaWarehouse::Tasks::SanityCheckServiceHistory.new(client_ids: @clients).run!
      return unless dirty
      ...
      @homeless_reset ||= service_history_enrollments_source.hud_residential_non_homeless.
        ...
        where(
          she_t[:move_in_date].not_eq(nil).and(
            she_t[:project_type].in(HudUtilityCurrent.residential_project_type_numbers_by_code[:ph]),
          ).or(
            she_t[:project_type].in(HudUtilityCurrent.residential_project_type_numbers_by_code[:th]),
          ),
        ).
    ...
    def residential_history_for_client(client_id:)
      ...
      @project_names = enrollments_by_project_entry.map do |_, e|
        e.map do |enrollment|
          ...
          project_type_id = ::HudUtilityCurrent.project_type_brief(enrollment[:project_type])
    ...
    def bed_night?(meta)
      meta[:project_type].in?(HudUtilityCurrent.project_type_number_from_code(:es_nbn))
    end
    ...
    def residential_dates enrollments:
      @non_homeless_types ||= HudUtilityCurrent.residential_project_type_numbers_by_code[:ph]
    ...
    def homeless_dates enrollments:
      @homeless_dates ||= enrollments.select do |e|
        e[:project_type].in? HudUtilityCurrent.chronic_project_types
      end.map do |e|
        e[:date]
      end.compact.uniq
    end
