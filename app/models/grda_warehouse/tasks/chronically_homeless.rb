...
module GrdaWarehouse::Tasks
  class ChronicallyHomeless
      ...
      @homeless_reset ||= service_history_enrollments_source.hud_residential_non_homeless.
        ...
        where(
          she_t[:move_in_date].not_eq(nil).and(
            she_t[:project_type].in(HudHelper.util.residential_project_type_numbers_by_code[:ph]),
          ).or(
            she_t[:project_type].in(HudHelper.util.residential_project_type_numbers_by_code[:th]),
          ),
        ).
    ...
    def residential_history_for_client(client_id:)
      ...
      @project_names = enrollments_by_project_entry.map do |_, e|
        e.map do |enrollment|
          ...
          project_type_id = ::HudHelper.util.project_type_brief(enrollment[:project_type])
    ...
    def bed_night?(meta)
      meta[:project_type].in?(HudHelper.util.project_type_number_from_code(:es_nbn))
    end
    ...
    def homeless_dates enrollments:
      @homeless_dates ||= enrollments.select do |e|
        e[:project_type].in? HudHelper.util.chronic_project_types
      end.map do |e|
        e[:date]
      end.compact.uniq
    end
