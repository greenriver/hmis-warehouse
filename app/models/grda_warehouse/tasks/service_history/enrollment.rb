...
module GrdaWarehouse::Tasks::ServiceHistory
  class Enrollment < GrdaWarehouse::Hud::Enrollment
    ...
    SO = Hud.util.residential_project_type_numbers_by_code[:so]
    ...
    def homeless?(date)
      return true if Hud.util.homeless_project_types.include?(project.project_type)
      return false if Hud.util.residential_project_type_numbers_by_code[:ph].include?(project.project_type) &&
    ...
    def literally_homeless? date
      return true if Hud.util.chronic_project_types.include?(project.project_type)
      return false if Hud.util.residential_project_type_numbers_by_code[:ph].include?(project.project_type) &&
        self.MoveInDate.present? && date > self.MoveInDate
      return false if Hud.util.residential_project_type_numbers_by_code[:th].include?(project.project_type)
