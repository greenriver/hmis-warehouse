...
module GrdaWarehouse::Tasks
  ...
  class EarliestResidentialService
      ...
      GrdaWarehouse::ServiceHistoryEnrollment.transaction do
        ...
        if to_add.any?
          to_add.each do |client_id, date|
            first_entry = service_history_source.entry.in_project_type(HudHelper.util.residential_project_type_ids).where(client_id: client_id, first_date_in_program: date).first
    ...
    def earliest_dates
      @earliest_dates ||= service_history_source.entry.in_project_type(HudHelper.util.residential_project_type_ids).group(:client_id).pluck(:client_id, Arel.sql('min(first_date_in_program)'))
    end
