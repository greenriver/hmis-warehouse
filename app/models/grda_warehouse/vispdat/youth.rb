...
module GrdaWarehouse::Vispdat
  class Youth < Base
    ['marijuana', 'violence_between_family_members', 'homeless_due_to_ran_away', 'homeless_due_to_religions_beliefs', 'homeless_due_to_family', 'homeless_due_to_gender_identity', 'incarcerated_before_18'].each do |field|
    ...
    def social_relationship_score
      if homeless_due_to_ran_away_answer_yes? ||
      homeless_due_to_religions_beliefs_answer_yes? ||
      homeless_due_to_family_answer_yes? ||
      homeless_due_to_gender_identity_answer_yes?
        1
      else
        0
      end
