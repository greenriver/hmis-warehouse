  ...
  class Base < Report
    ...
    private def missing_housing_types(user)
      ...
      missing_data_rows(
        ...
        includes(:funders).
        where(project_type: [1, 2, 3, 8, 9, 10, 13]).
    ...
    private def operating_start_dates(user)
      missing_data_rows(
        GrdaWarehouse::Hud::Project.viewable_by(user, permission: :can_view_assigned_reports).coc_funded.joins(:organization).
        includes(:funders).
        where(project_type: [1, 2, 3, 8, 9, 10, 13, 4]).
    ...
    private def invalid_funders(user)
      missing_data_rows(
        ...
        where(f_t[:Funder].not_in(::HudHelper.util('legacy').funding_sources.keys).or(f_t[:GrantID].eq(nil))),
      )
    end
