...
module Reporting
  class Housed < ReportingBase
    ...
    scope :rrh, -> do
      where(project_type: HudUtilityCurrent.performance_reporting[:rrh])
    end
    ...
    scope :es, -> do
      where(project_type: HudUtilityCurrent.performance_reporting[:es])
    end

    scope :th, -> do
      where(project_type: HudUtilityCurrent.performance_reporting[:th])
    end

    scope :sh, -> do
      where(project_type: HudUtilityCurrent.performance_reporting[:sh])
    end
    ...
    scope :ph_destinations, -> do
      where(destination: ::HudUtilityCurrent.permanent_destinations)
    end
    ...
    def self.available_races
      ::HudUtilityCurrent.races(multi_racial: true)
    end

    def self.available_genders
      ::HudUtilityCurrent.genders
    end

    def self.available_veteran_stati
      ::HudUtilityCurrent.no_yes_reasons_for_missing_data_options
    end
      ...
      Reporting::Housed.with_advisory_lock(ADVISORY_LOCK_KEY, timeout_seconds: 0) do
        ...
        client_ids.each_slice(1_000) do |client_id_batch|
          ...
          data = enrollment_data(client_id_batch).map do |en|
            ...
            if HudUtilityCurrent.permanent_destinations.include?(en[:destination])
    ...
    def two_project_data(client_id_batch)
      ...
      from_residential_enrollments = two_project_residential_data(client_id_batch).map do |residential_enrollment|
        ...
        when *HudUtilityCurrent.residential_project_type_numbers_by_code[:ph]
          key = [
