...
module Reporting::DataQualityReports
  class Enrollment < ReportingBase
    ...
    scope :ph, -> do
      where project_type: HudUtilityCurrent.residential_project_type_numbers_by_code[:ph]
    end
    ...
    def calculate_ssn_missing ssn:
      ssn.blank? || ! ::HudUtilityCurrent.valid_social?(ssn)
    end
    ...
    def calculate_days_in_ph_before_move_in_date project_type:, entry_date:, move_in_date:, report_end:
      return nil unless project_type.in?(HudUtilityCurrent.residential_project_type_numbers_by_code[:ph])
