class NotifyUser < DatabaseMailer
  ...
  def health_member_status_report_finished user_id
    ...
    mail(from: '"Boston Coordinated Care Hub" <cas-help@cas.boston.gov>',to: @user.email, subject: "Your Member Status report has finished")
  end

  def health_claims_finished user_id
    @user = User.find(user_id)
    @report_url = warehouse_reports_health_claims_url
    mail(from: '"Boston Coordinated Care Hub" <cas-help@cas.boston.gov>', to: @user.email, subject: "Your Claims file has been generated")
  end

  def health_qa_pre_calculation_finished user_id
    @user = User.find(user_id)
    @report_url = warehouse_reports_health_claims_url
    mail(from: '"Boston Coordinated Care Hub" <cas-help@cas.boston.gov>', to: @user.email, subject: "Qualifying Activity Payability has been calculated")
  end
