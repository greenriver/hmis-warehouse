...
class NotifyUser < DatabaseMailer
  ...
  def metric_threshold_crossed(user_id:, crossings:, calculation_date:)
    @user = User.find(user_id)
    return unless @user.active?

    @crossings = crossings
    @calculation_date = calculation_date
    @metric_definitions = GrdaWarehouse::Monitoring::MetricDefinition.
      where(id: crossings.keys).
      index_by(&:id)

    mail(to: @user.email, subject: 'Metric Threshold Monitoring Alert')
  end
end
