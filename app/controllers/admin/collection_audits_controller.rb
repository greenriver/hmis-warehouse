...
module Admin
  class CollectionAuditsController < ::ApplicationController
    ...
    def export
      respond_to do |format|
        format.csv do
          send_data generate_audit_csv(@versions, @history), filename: "#{@collection.name.parameterize}-audit-history-#{Date.current.to_fs(:db)}.csv"
        end
      end
    end
