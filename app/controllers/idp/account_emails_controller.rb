...
class Idp::AccountEmailsController < ApplicationController
  ...
  # confirmation that isn't here at all.
  private def reconcile_email_from_idp
    ...
    # rolls back the adopted address instead of leaving HMIS rows keyed on an email we no longer hold.
    ...
    # Nothing was written, so there's no state to roll back and no reason to fail the page. flash.now,
  ...
  # Token holder, not current_user: under impersonation these differ, and begin_change hands the
