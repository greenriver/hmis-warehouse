...
module WarehouseReports::Health
  class ContactTracingController < ApplicationController
    ...
    def single_case
      single_case = Health::Tracing::Case.find(params[:id].to_i)
      @cases = Health::Tracing::Case.where(id: single_case.id)
      render xlsx: 'download', filename: "#{single_case.name}.xlsx"
    end
      ...
      @index_cases ||= @cases.map do |index_case|
        {
          ...
          where_person_sleeps: index_case.locations[0]&.location,
          other_location_1: index_case.locations[1]&.location,
          other_location_2: index_case.locations[2]&.location,
          other_location_3: index_case.locations[3]&.location,
          other_location_4: index_case.locations[4]&.location,
          occupation: index_case.occupation,
          recent_incarceration: index_case.recent_incarceration,
          additional_locations: index_case.locations[5..]&.map(&:location)&.join(', '),
