...
# GrdaWarehouse::ClientRetentionMark; every mark and unmark is logged with identifiers only.
...
class ClientRetentionJob < BaseJob
  ...
  private def process_batch(destination_ids)
    rows = GrdaWarehouse::ClientRetentionMark.rollup_activity(destination_ids: destination_ids, global_years: @global_years)
    ...
    marked_source_ids = GrdaWarehouse::ClientRetentionMark.where(client_id: evaluated_source_ids).pluck(:client_id).to_set
