...
class BackgroundRender::DisabilitySummaryJob < BackgroundRenderJob
  def render_html(partial:, filters:, user_id:)
    ...
    @section = 'disabilities' if @section.blank? && params.require(:partial) == 'disabilities'

    raise 'Rollup not in allowlist' unless @section.present?

    @section = @report.section_subpath + @section
    DisabilitySummary::WarehouseReports::DisabilitySummaryController.render(
      partial: @section,
      assigns: {
        report: @report,
        section: @section,
