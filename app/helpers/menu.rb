...
module Menu
  ...
  def clients_menu
    ...
    title = if can_view_aggregate_health? || can_view_patients_for_own_agency?
  ...
  def data_menu
    ...
    path = data_sources_path if (can_view_imports_projects_or_organizations? || can_upload_hud_zips? || can_manage_some_ad_hoc_ds?) && (can_view_projects? || can_view_organizations? || can_upload_hud_zips?)
    menu.add_child(
      Menu::Item.new(
        user: current_user,
  ...
  def administration_menu
    ...
    menu.add_child(
      Menu::Item.new(
        ...
        path: current_user.admin_dashboard_landing_path,
        title: Translation.translate('Warehouse Admin'),
      ),
    )
    path = if current_user.can_approve_patient_assignments?
      review_admin_health_patient_referrals_path
    elsif (current_user.can_manage_health_agency? || current_user.can_manage_patients_for_own_agency?) && current_user.health_agencies.any?
    ...
    menu.add_child(
      Menu::Item.new(
        user: current_user,
        visible: ->(_user) { hmis_admin_visible? },
    ...
    if help_for_path
      ...
      menu.add_child(
        Menu::Item.new(
          user: current_user,
          visible: ->(user) { user.can_edit_help? && !help_for_path },
          path: new_help_path(controller_path: controller_path, action_name: action_name),
