...
module ApplicationHelper
  ...
  def ssn(number)
    if can_view_full_ssn?
      ...
      content_tag :span, number.to_s.gsub(HudUtilityCurrent::SSN_RGX, '\1-\2-\3')
    else
      masked_ssn(number)
    end
  end
  ...
  def masked_ssn(number, include_content_tag: true)
    # pad with leading 0s if we don't have enough characters
    number = number.to_s.rjust(9, '0') if number.present?
    value = number.to_s.gsub(HudUtilityCurrent::SSN_RGX, 'XXX-XX-\3')
    return value unless include_content_tag

    content_tag :span, number.to_s.gsub(HudUtilityCurrent::SSN_RGX, 'XXX-XX-\3')
  end
