...
module HmisCsvTwentyTwenty::Importer
  class Importer
    ...
    def initialize( # rubocop:disable Metrics/ParameterLists
      ...
      @loader = HmisCsvTwentyTwenty::Loader::Loader.find(loader_id.to_i)
    ...
    def pre_process!
      ...
      importable_files.each do |file_name, klass|
        batch = []
        source_data_scope_for.find_each(batch_size: 10_000) do |source|
          destination = klass.new_from(source)
          destination.importer_log_id = @importer_log.id
          destination.pre_processed_at = Time.current
          destination.set_processed_as
          destination.fix_date_columns
    ...
    private def source_data_scope_for(file_name)
      @loaded_files ||= @loader.importable_files
      @loaded_files[file_name].where(loader_id: @loader.id)
    end
    ...
    def start_import
      ...
      @loader.update(importer_log_id: @importer_log.id)
    end
