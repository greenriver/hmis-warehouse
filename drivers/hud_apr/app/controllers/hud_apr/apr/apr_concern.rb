...
module HudApr::Apr::AprConcern
  ...
  included do
    ...
    private def path_for_running_all_questions
      running_all_questions_hud_reports_aprs_path({ skip_trackable: true }.merge(link_params.except('action', 'controller')))
    end

    private def path_for_running_question
      running_hud_reports_aprs_path({ skip_trackable: true }.merge(link_params.except('action', 'controller')))
    end
