...
module HudApr::Apr::CeAprConcern
  ...
  included do
    ...
    private def path_for_question(question, report: nil)
      hud_reports_ce_apr_question_path(apr_id: report&.id || 0, id: question)
    end

    private def path_for_questions(question)
      hud_reports_ce_apr_questions_path(apr_id: 0, question: question)
    end

    private def path_for_question_result(question, report: nil)
      result_hud_reports_ce_apr_question_path(apr_id: report&.id || 0, id: question)
    end
    ...
    private def path_for_cell(report:, question:, cell_label:, table:)
      hud_reports_ce_apr_question_cell_path(apr_id: report&.id || 0, question_id: question, id: cell_label, table: table)
    end
  end
end
