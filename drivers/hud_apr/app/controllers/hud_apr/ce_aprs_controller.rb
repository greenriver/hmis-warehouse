...
module HudApr
  class CeAprsController < BaseController
    ...
    def index
      ...
      @path_for_running = running_all_questions_hud_reports_ce_aprs_path
    end
    ...
    def show
      respond_to do |format|
        format.html do
          ...
          @path_for_running = running_hud_reports_ce_aprs_path(link_params.except('action', 'controller'))
        end
        format.zip do
          exporter = ::HudReports::ZipExporter.new(@report)
          date = Date.current.strftime('%Y-%m-%d')
          send_data exporter.export!, filename: "ce-apr-#{date}.zip"
        end
      end
    end
    ...
    def create
      if @filter.valid?
        ...
        redirect_to history_hud_reports_ce_aprs_path
      else
        render :new
      end
    end
