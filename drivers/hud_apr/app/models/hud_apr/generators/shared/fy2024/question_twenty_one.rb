    ...
    private def health_insurance_counts(suffix)
      {
        ...
        'Veteran’s Health Administration (VHA)' => { hud_report_apr_clients: { "income_sources_at_#{suffix}" => { VHAServices: 1 } } },
        ...
        label_for(:dkptr) => a_t["insurance_from_any_source_at_#{suffix}"].in([8, 9]).
          and(insurance_jsonb_clause(1, a_t["income_sources_at_#{suffix}"].to_sql, negation: true)),
        label_for(:data_not_collected) => a_t["insurance_from_any_source_at_#{suffix}"].eq(99).
