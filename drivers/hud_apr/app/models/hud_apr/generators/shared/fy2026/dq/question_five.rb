...
module HudApr::Generators::Shared::Fy2026::Dq::QuestionFive
  ...
  included do
    private def generate_q5(table_name)
      ...
      es_sh_so_projects = HudHelper.util('2026').performance_reporting[:es] + HudHelper.util('2026').performance_reporting[:sh] + HudHelper.util('2026').performance_reporting[:so]
      th_projects = HudHelper.util('2026').performance_reporting[:th]
      ph_projects = HudHelper.util('2026').performance_reporting[:ph]
      ce_projects = HudHelper.util('2026').performance_reporting[:ce]
