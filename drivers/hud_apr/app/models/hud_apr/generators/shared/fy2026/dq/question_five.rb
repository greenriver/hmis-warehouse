...
module HudApr::Generators::Shared::Fy2026::Dq::QuestionFive
  ...
  included do
    private def generate_q5(table_name)
      ...
      es_sh_so_projects = Hud.util('2026').performance_reporting[:es] + Hud.util('2026').performance_reporting[:sh] + Hud.util('2026').performance_reporting[:so]
      th_projects = Hud.util('2026').performance_reporting[:th]
      ph_projects = Hud.util('2026').performance_reporting[:ph]
      ce_projects = Hud.util('2026').performance_reporting[:ce]
      sso_ds_hp_projects = Hud.util('2026').performance_reporting[:services_only] + Hud.util('2026').performance_reporting[:day_shelter] + Hud.util('2026').performance_reporting[:prevention]
