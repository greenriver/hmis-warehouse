...
RSpec.describe HudApr::Generators::CeApr::Fy2021::QuestionFive, type: :model do
  ...
  describe 'with no assessments' do
    describe 'with default filters' do
      before(:all) do
        ...
        run(default_filter, HudApr::Generators::CeApr::Fy2021::QuestionFive::QUESTION_NUMBER)
      end
    ...
    describe 'When filtering with a specific race' do
      before(:all) do
        default_setup(default_setup_path)
        run(race_filter, HudApr::Generators::CeApr::Fy2021::QuestionFive::QUESTION_NUMBER)
      end
      it 'counts people served' do
        expect(report_result.answer(question: 'Q5a', cell: 'B1').summary).to eq(0)
      end
    ...
    describe 'When filtering with a specific age range' do
      before(:all) do
        default_setup(default_setup_path)
        run(age_filter, HudApr::Generators::CeApr::Fy2021::QuestionFive::QUESTION_NUMBER)
      end
      it 'counts people served' do
        expect(report_result.answer(question: 'Q5a', cell: 'B1').summary).to eq(0)
      end
