...
RSpec.shared_context 'datalab organization y sso apr', shared_context: :metadata do
  describe 'Datalab 2026 APR - Organization Y - SSO' do
    let(:results_dir) { 'apr/organization_y_sso' }
    before(:all) do
      puts
      puts 'Running APR Organization Y - SSO'
      generator = HudApr::Generators::Apr::Fy2026::Generator
      project_ids = GrdaWarehouse::Hud::Project.where(ProjectName: ['Organization Y - SSO']).pluck(:id)
        ...
        aggregate_failures do
          apr_validations.each do |question, table_validations|
            table_validations.each do |validation|
              ...
              next unless validation[:source][:relevant_project_types]&.include?(6)
...
RSpec.configure do |rspec|
  rspec.include_context 'datalab organization y sso apr', include_shared: true
end
