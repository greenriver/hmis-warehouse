...
module HmisDataQualityTool
  class Client < ::HudReports::ReportClientBase
    ...
    def self.detail_headers
      {
        ...
        name_data_quality: { title: 'Name Data Quality', translator: ->(v) { "#{HudUtilityCurrent.name_data_quality(v)} (#{v})" } },
        personal_id: { title: 'HMIS Personal ID' },
        dob: { title: 'DOB' },
        dob_data_quality: { title: 'DOB Data Quality', translator: ->(v) { "#{HudUtilityCurrent.dob_data_quality(v)} (#{v})" } },
        man: { title: 'Man', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v)} (#{v})" } },
        woman: { title: 'Woman', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v)} (#{v})" } },
        culturally_specific: { title: 'Culturally Specific', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v)} (#{v})" } },
        different_identity: { title: 'DifferentIdentity', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v)} (#{v})" } },
        non_binary: { title: 'Non-Binary', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v)} (#{v})" } },
        transgender: { title: 'Transgender', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v)} (#{v})" } },
        questioning: { title: 'Questioning', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v)} (#{v})" } },
        gender_none: { title: 'Gender None', translator: ->(v) { "#{HudUtilityCurrent.gender_none(v)} (#{v})" } },
        am_ind_ak_native: { title: 'American Indian, Alaska Native, or Indigenous', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v&.to_i)} (#{v})" } },
        asian: { title: 'Asian or Asian American', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v&.to_i)} (#{v})" } },
        black_af_american: { title: 'Black, African American, or African', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v&.to_i)} (#{v})" } },
        native_hi_pacific: { title: 'Native Hawaiian or Pacific Islander', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v&.to_i)} (#{v})" } },
        white: { title: 'White', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v&.to_i)} (#{v})" } },
        mid_east_n_african: { title: 'Middle Eastern or North African', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v&.to_i)} (#{v})" } },
        hispanic_latinaeo: { title: 'Hispanic/Latina/e/o', translator: ->(v) { "#{HudUtilityCurrent.no_yes(v&.to_i)} (#{v})" } },
        race_none: { title: 'Race None', translator: ->(v) { "#{HudUtilityCurrent.race_none(v)} (#{v})" } },
        veteran_status: { title: 'Veteran Status', translator: ->(v) { "#{HudUtilityCurrent.no_yes_reasons_for_missing_data(v)} (#{v})" } },
        ssn: { title: 'SSN', translator: ->(v) { masked_ssn(v) } },
        ssn_data_quality: { title: 'SSN Data Quality', translator: ->(v) { "#{HudUtilityCurrent.ssn_data_quality(v)} (#{v})" } },
    ...
    def self.masked_ssn(number)
      ...
      number.to_s.gsub(HudUtilityCurrent.current_hud_utility::SSN_RGX, 'XXX-XX-\3')
    end
    ...
    def self.sections(_)
      {
        gender_issues: {
          ...
          limiter: ->(item) {
            ...
            return true if (values - HudUtilityCurrent.no_yes_options.keys).any?
