  ...
  describe 'table scaffolding' do
    ...
    it 'builds the return table without data' do
      ['B2', 'C2', 'G7', 'J7'].each do |cell|
        expect(@report.answer(question: '6a.1 and 6b.1', cell: cell).summary.to_i).to eq(0)
      end
    end

    it 'builds the category 3 placement tables' do
      expect(@report.answer(question: '6c.1', cell: 'C2').summary.to_i).to eq(0)
      expect(@report.answer(question: '6c.2', cell: 'C2').summary.to_i).to eq(0)
    end
  end
end
