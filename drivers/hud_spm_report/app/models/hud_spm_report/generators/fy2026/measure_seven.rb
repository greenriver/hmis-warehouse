...
module HudSpmReport::Generators::Fy2026
  class MeasureSeven < MeasureBase
    ...
    def create_so_universe
      ...
      so_enrollments = enrollment_set.open_during_range(filter.range).where(project_type: HudHelper.util('2026').project_type_number_from_code(:so))
    ...
    def create_ph_no_move_in_universe
      ...
      enrollments = HudSpmReport::Fy2026::SpmEnrollment.one_for_column(:exit_date, source_arel_table: spm_e_t, group_on: :client_id, scope: leavers)
      ph_not_rrh = HudHelper.util('2026').project_type_number_from_code(:ph) - HudHelper.util('2026').project_type_number_from_code(:rrh)
