...
module HudSpmReport::Generators::Fy2026
  class MeasureOne < MeasureBase
    ...
    private def run_1a(table_name)
      ...
      create_universe(
        :m1a1,
        included_project_types: HudHelper.util('2026').project_type_number_from_code(:es) + HudHelper.util('2026').project_type_number_from_code(:sh),
        excluded_project_types: HudHelper.util('2026').project_type_number_from_code(:th) + HudHelper.util('2026').project_type_number_from_code(:ph),
      )
      ...
      create_universe(
        :m1a2,
        included_project_types: HudHelper.util('2026').project_type_number_from_code(:es) +
          HudHelper.util('2026').project_type_number_from_code(:sh) +
          HudHelper.util('2026').project_type_number_from_code(:th),
        excluded_project_types: HudHelper.util('2026').project_type_number_from_code(:ph),
      )
    ...
    private def run_1b(table_name)
      ...
      create_universe(
        ...
        included_project_types: HudHelper.util('2026').project_type_number_from_code(:es) +
          HudHelper.util('2026').project_type_number_from_code(:sh),
        excluded_project_types: HudHelper.util('2026').project_type_number_from_code(:th) +
          HudHelper.util('2026').project_type_number_from_code(:ph),
    ...
    private def create_universe(universe_name, included_project_types:, excluded_project_types:, include_self_reported_and_ph: false)
      ...
      if include_self_reported_and_ph
        ...
        literally_homeless_in_ph = enrollment_set.literally_homeless_at_entry_in_range(filter.range).where(project_type: HudHelper.util('2026').project_type_number_from_code(:ph))
    ...
    private def compute_row(universe)
      ...
      return [0, '0.00', '0.00'] unless persons.positive?
    ...
    private def median(values)
      ...
      if len.even?
        ...
        median_val = (sorted[len / 2 - 1] + sorted[len / 2]) / 2.0
      else
        median_val = sorted[len / 2]
      end
