...
module HudSpmReport::Generators::Fy2026
  class MeasureFive < MeasureBase
    ...
    private def run_5_1(table_name)
      ...
      report_members = create_universe(:m5_1, [:es, :sh, :th].map { |code| Hud.util('2026').project_type_number_from_code(code) }.flatten)
    ...
    private def run_5_2(table_name)
      ...
      report_members = create_universe(:m5_2, [:es, :sh, :th, :ph].flat_map { |code| Hud.util('2026').project_type_number_from_code(code) })
      answer = @report.answer(question: table_name, cell: 'C2')
      answer.add_members(report_members)
      answer.update(summary: report_members.count)
