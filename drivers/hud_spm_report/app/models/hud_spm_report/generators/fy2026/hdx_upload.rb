...
module HudSpmReport::Generators::Fy2026
  ...
  class HdxUpload < MeasureBase
    ...
    def dq(section, table_name, cell_name)
      ...
      when :essh
        @reports[section] ||= generate_dq(HudHelper.util('2026').residential_project_type_numbers_by_codes(:es, :sh))
      when :th
        @reports[section] ||= generate_dq(HudHelper.util('2026').residential_project_type_numbers_by_codes(:th))
      when :pshoph
        @reports[section] ||= generate_dq(HudHelper.util('2026').residential_project_type_numbers_by_codes(:psh, :oph))
      when :rrh
        @reports[section] ||= generate_dq(HudHelper.util('2026').residential_project_type_numbers_by_codes(:rrh))
      when :so
        @reports[section] ||= generate_dq(HudHelper.util('2026').residential_project_type_numbers_by_codes(:so))
      else
    ...
    private def generate_dq(project_types)
      ...
      dq_filter.report_version = :fy2026
