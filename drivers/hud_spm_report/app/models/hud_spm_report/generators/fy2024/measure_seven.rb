...
module HudSpmReport::Generators::Fy2024
  class MeasureSeven < MeasureBase
    ...
    def create_so_universe
      ...
      so_enrollments = enrollment_set.open_during_range(filter.range).where(project_type: Hud.util('2024').project_type_number_from_code(:so))
    ...
    def create_ph_no_move_in_universe
      ...
      project_types = [:es, :sh, :th, :ph].flat_map { |code| Hud.util('2024').project_type_number_from_code(code) }
      ...
      enrollments = HudSpmReport::Fy2024::SpmEnrollment.one_for_column(:exit_date, source_arel_table: spm_e_t, group_on: :client_id, scope: leavers)
      ph_not_rrh = Hud.util('2024').project_type_number_from_code(:ph) - Hud.util('2024').project_type_number_from_code(:rrh)
