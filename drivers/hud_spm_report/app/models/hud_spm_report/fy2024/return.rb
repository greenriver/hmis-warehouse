  ...
  class Return < HudReports::ReportClientBase
    ...
    def self.client_ids_with_permanent_exits(report, enrollments)
      ...
      enrollments.where(exit_date: filter.start - 730.days .. filter.end - 730.days).
        where(destination: Hud.util('2024').permanent_destinations).
        pluck(:client_id).
        uniq
    end
    ...
    def compute_return(enrollments)
      ...
      self.return_enrollment = candidate_returns.detect do |enrollment|
        ...
        enrollment.project_type.in?(Hud.util('2024').homeless_project_type_numbers) ||
          (enrollment.project_type.in?(Hud.util('2024').project_type_number_from_code(:ph)) &&
