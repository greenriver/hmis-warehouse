  ...
  class Episode < HudReports::ReportClientBase
    ...
    def compute_episode(enrollments, included_project_types:, excluded_project_types:, include_self_reported_and_ph:)
      ...
      pre_entry_inclusion_dates = if include_self_reported_and_ph
        ph_enrollments = enrollments.select { |enrollment, _, _| enrollment.project_type.in?(HudHelper.util('2024').project_type_number_from_code(:ph)) }
    ...
    private def candidate_bed_nights(enrollments, project_types, include_self_reported_and_ph)
      ...
      enrollments = enrollments.select do |e|
        ...
        if include_self_reported_and_ph && e.project_type.in?(HudHelper.util('2024').project_type_number_from_code(:ph))
      ...
      enrollments.each do |enrollment|
        if enrollment.project_type.in?(HudHelper.util('2024').project_type_number_from_code(:es_nbn))
          ...
          end_date = if enrollment.project_type.in?(HudHelper.util('2024').project_type_number_from_code(:ph))
    ...
    private def excluded_bed_nights(enrollments, project_types)
      ...
      enrollments.each do |enrollment|
        if enrollment.project_type.in?(HudHelper.util('2024').project_type_number_from_code(:th))
        ...
        elsif enrollment.project_type.in?(HudHelper.util('2024').project_type_number_from_code(:ph))
