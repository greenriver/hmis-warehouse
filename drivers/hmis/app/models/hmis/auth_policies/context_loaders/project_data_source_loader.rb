...
module Hmis::AuthPolicies::ContextLoaders
  class ProjectDataSourceLoader
    ...
    def preload(project_ids)
      ...
      results = Hmis::Hud::Project.where(id: new_project_ids).pluck(:id, :data_source_id).to_h
      @cache.merge!(results)

      # For projects that don't exist, add `nil` to the cache so we don't check the db again
