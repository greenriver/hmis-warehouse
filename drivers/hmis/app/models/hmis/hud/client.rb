...
class Hmis::Hud::Client < Hmis::Hud::Base
  ...
  def build_primary_custom_client_name
    ...
    build_custom_client_name_from_client_record
  end

  def build_custom_client_name_from_client_record
    names.new(
      primary: true,
      first: first_name,
      last: last_name,
  ...
  use_enum(:race_enum_map, ::HudHelper.util('2026').races.except('RaceNone'), include_base_null: true) do |hash|
    hash.map do |value, desc|
      {
        key: value,
        value: value,
