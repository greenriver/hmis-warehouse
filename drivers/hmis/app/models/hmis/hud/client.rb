...
class Hmis::Hud::Client < Hmis::Hud::Base
  ...
  use_enum(:gender_enum_map, ::Hud.util('2026').genders) do |hash|
    hash.map do |value, desc|
      {
        key: [8, 9, 99].include?(value) ? desc : ::Hud.util('2026').gender_id_to_field_name[value],
        value: value,
        desc: desc,
  ...
  use_enum(:race_enum_map, ::Hud.util('2026').races.except('RaceNone'), include_base_null: true) do |hash|
    hash.map do |value, desc|
      {
        key: value,
        value: value,
