...
class Hmis::Hud::Validators::ClientValidator < Hmis::Hud::Validators::BaseValidator
  ...
  def validate(record)
    super(record) do
      record.errors.add :gender, :required if !skipped_attributes(record).include?(:gender) && ::HudUtilityCurrent.gender_id_to_field_name.except(8, 9, 99).values.any? { |field| record.send(field).nil? }
      record.errors.add :race, :required if !skipped_attributes(record).include?(:race) && ::HudUtilityCurrent.races.except('RaceNone').keys.any? { |field| record.send(field).nil? }
