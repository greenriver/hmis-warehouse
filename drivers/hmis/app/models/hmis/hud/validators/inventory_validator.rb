...
class Hmis::Hud::Validators::InventoryValidator < Hmis::Hud::Validators::BaseValidator
  ...
  def validate(record)
    super(record) do
      ...
      record.errors.add :coc_code, :invalid, message: 'is invalid' if record.coc_code.present? && (!Hud.util.valid_coc?(record.coc_code) || !Hmis::Hud::ProjectCoc.where(coc_code: record.coc_code, project_id: record.project_id).exists?)
