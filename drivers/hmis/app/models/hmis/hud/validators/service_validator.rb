...
class Hmis::Hud::Validators::ServiceValidator < Hmis::Hud::Validators::BaseValidator
  ...
  TYPE_PROVIDED_MAP = ::HudHelper.util.service_types_provided_map.map { |k, v| [k, v[:list]] }.to_h.freeze

  SUB_TYPE_PROVIDED_MAP = ::HudHelper.util.service_sub_types_provided_map.map { |k, v| [k, v[:list]] }.to_h.freeze
  ...
  def self.validate_service_type(record, record_type_field: :record_type, type_provided_field: :type_provided)
    ...
    unless HudHelper.util.record_types.any? { |k, _v| record_type == k }
