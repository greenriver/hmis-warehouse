class Hmis::Hud::Validators::ServiceValidator < Hmis::Hud::Validators::BaseValidator
  ...
  def validate_sub_type_provided(record)
    return record.errors.add :sub_type_provided, :invalid, message: 'Invalid SubTypeProvided for RecordType', full_message: "SubTypeProvided must be null unless RecordType = 144. RecordType is #{record.record_type}" unless record.record_type == 144
    ...
    record.errors.add :type_provided, :invalid, message: 'Invalid SubTypeProvided for TypeProvided', full_message: "Value for SubTypeProvided '#{record.sub_type_provided}' does not match TypeProvided '#{record.type_provided}'"
  end
end
