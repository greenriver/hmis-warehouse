  ...
  ::Hmis::Role.permissions.each do |permission|
    ...
    define_method("#{permission}_for?") do |entity|
      ...
      access_group_ids = nil
       ...
       access_group_ids = if entity.is_a?(Hmis::Hud::Project)
       Hmis::GroupViewableEntity.includes_project(entity).pluck(:access_group_id)
      elsif entity.is_a?(Hmis::Hud::Organization)
        Hmis::GroupViewableEntity.includes_organization(entity).pluck(:access_group_id)
      end
