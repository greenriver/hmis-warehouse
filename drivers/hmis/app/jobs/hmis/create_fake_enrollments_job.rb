  ...
  class CreateFakeEnrollmentsJob < BaseJob
    ...
    def build_fake_enrollment(client:, project:, is_hoh:)
      ...
      enrollment = Hmis::Hud::Enrollment.new(
        **hud_attributes,
        ...
        LivingSituation: HudUtilityCurrent.prior_living_situations.keys.sample,
      )
      # rubocop:disable Style/IfUnlessModifier
      if is_hoh && HudUtilityCurrent.doe_project_types.include?(project.ProjectType) && rand_boolean(10)
        enrollment.DateOfEngagement = [enrollment.EntryDate + rand(30), today].min
      end
      if is_hoh && HudUtilityCurrent.permanent_housing_project_types.include?(project.ProjectType) && rand_boolean(10)
    ...
    def build_fake_exit(enrollment)
      ...
      Hmis::Hud::Exit.new(
        ...
        exit_date: exit_date,
        destination: HudUtilityCurrent.destinations.keys.excluding(17).sample,
    ...
    def build_fake_disabilities(enrollment)
      HudUtilityCurrent.disability_types.keys.map.each do |type|
    ...
    def random_race_attributes
      race_attributes = HudUtilityCurrent.races.keys.excluding('RaceNone').each_with_object(0).to_h
    ...
    def random_gender_attributes
      gender_attributes = HudUtilityCurrent.gender_fields.map(&:to_s).excluding('GenderNone').each_with_object(0).to_h
