  ...
  class Forms::PickListOption < Types::BaseObject
    ...
    def self.options_for_type(pick_list_type, user:, project_id: nil, client_id: nil, household_id: nil)
      ...
      when 'RESIDENTIAL_PROJECTS'
        Hmis::Hud::Project.viewable_by(user).
          ...
          where(project_type: HudUtilityCurrent.residential_project_type_ids).
          preload(:organization).
          sort_by_option(:organization_and_name).
          map(&:to_pick_list_option)
      when 'OPEN_PROJECTS'
    ...
    def self.static_options_for_type(pick_list_type, user:)
      ...
      when 'OTHER_FUNDERS'
        Hmis::Hud::Funder.where(data_source_id: user.hmis_data_source_id).
          where(Funder: HudUtilityCurrent.local_or_other_funding_source).where.not(OtherFunder: nil).
    ...
    def self.coc_picklist(selected_project)
      available_codes = if selected_project.present?
        selected_project.project_cocs.pluck(:CoCCode).uniq.map { |code| [code, ::HudUtilityCurrent.cocs[code] || code] }
      else
        ::HudUtilityCurrent.cocs_in_state(GrdaWarehouse::Config.relevant_state_codes)
      end
    ...
    def self.living_situation_picklist(as:)
      ...
      [
        [HudUtilityCurrent.current_hud_utility::SITUATION_HOMELESS_RANGE, :HOMELESS, 'Homeless Situations'],
        [HudUtilityCurrent.current_hud_utility::SITUATION_INSTITUTIONAL_RANGE, :INSTITUTIONAL, 'Institutional Situations'],
        [HudUtilityCurrent.current_hud_utility::SITUATION_TEMPORARY_RANGE, :TEMPORARY, 'Temporary Housing Situations'],
        [HudUtilityCurrent.current_hud_utility::SITUATION_PERMANENT_RANGE, :PERMANENT, 'Permanent Housing Situations'],
        [HudUtilityCurrent.current_hud_utility::SITUATION_OTHER_RANGE, :OTHER, 'Other'],
        ...
        enum_value_definitions.select { |e| range.include? e.value }.map do |enum|
          {
            ...
            label: ::HudUtilityCurrent.living_situation(enum.value),
