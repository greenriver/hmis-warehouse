...
module Mutations
  class AcHmis::UpdateReferralPosting < CleanBaseMutation
    ...
    def resolve(id:, input:)
      ...
      handle_error('access denied') unless current_user.policy_for(posting.project, policy_type: :hmis_project).can_view_enrollment_details?
      ...
      with_logging_transaction(posting) do |logger|
        ...
        if errors.empty? && posting_status_change == ['assigned_status', 'accepted_pending_status']
          ...
          if posting.unit_type_id
            unit_to_assign = posting.project&.units&.unoccupied_on&.not_receiving_referrals&.find_by(unit_type_id: posting.unit_type_id)
