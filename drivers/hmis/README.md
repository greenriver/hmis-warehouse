...
**3. Set up frontend env** (optional). In frontend env, set `HMIS_HOSTNAME=hmis.dev.test,hmis-2.dev.test` so the shared cert generated by `vite-plugin-mkcert` includes both hostnames. The plugin writes one cert to `~/.vite-plugin-mkcert/` and overwrites it each time a dev server starts; if each server only passed its own host, the last server to start would replace the cert and the other would show a cert mismatch after a restart. Passing all hostnames produces a single cert valid for both.
