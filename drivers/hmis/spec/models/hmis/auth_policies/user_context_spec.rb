...
RSpec.describe Hmis::AuthPolicies::UserContext, type: :model do
  ...
  describe '#project_permissions' do
    ...
    context 'when the user is not tied to a data source' do
      before do
        user.hmis_data_source_id = nil
      end

      it 'raises an error' do
        expect { Hmis::AuthPolicies::UserContext.new(user) }.to raise_error(ArgumentError, /tied to an HMIS data source/i)
      end
    end
  end
end
