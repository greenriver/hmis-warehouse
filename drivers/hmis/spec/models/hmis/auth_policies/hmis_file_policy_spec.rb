...
RSpec.describe Hmis::AuthPolicies::HmisFilePolicy, type: :model do
  ...
  describe 'Instance policy' do
    ...
    describe '#can_manage? and #can_delete?' do
      context 'when user can only view' do
        let!(:access_control) { create_access_control(user, project, with_permission: [:can_view_clients, :can_view_any_nonconfidential_client_files, :can_view_any_confidential_client_files]) }

        it 'denies manage access' do
          expect(file_policy(file).can_manage?).to be false
          expect(file_policy(confidential_file).can_manage?).to be false
          expect(file_policy(own_file).can_manage?).to be false
        end
      end
      ...
      context 'when user can manage own files' do
        ...
        it 'grants manage access to own files' do
          expect(file_policy(own_file).can_manage?).to be true
          expect(file_policy(file).can_manage?).to be false
          expect(file_policy(confidential_file).can_manage?).to be false
        end
      end
      ...
      context 'when user can manage any files in the project (meaning all files they can view, they can manage)' do
        ...
        it 'grants manage access to all files in the project' do
          expect(file_policy(file).can_manage?).to be true
          expect(file_policy(confidential_file).can_manage?).to be true
          expect(file_policy(own_file).can_manage?).to be true
        end
      end
      ...
      context 'when user can manage any and view non-confidential but the file is confidential' do
        ...
        it 'only allows managing non-confidential files' do
          expect(file_policy(confidential_file).can_manage?).to be false
        end
      end
    end
