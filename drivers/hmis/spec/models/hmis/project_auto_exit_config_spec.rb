...
RSpec.describe Hmis::ProjectAutoExitConfig, type: :model do
  ...
  it 'should select the proper auto exit config for a project' do
    ...
    expect(Hmis::ProjectAutoExitConfig.detect_best_config_for_project(p2)).to eq(aec2)
    ...
    expect(Hmis::ProjectAutoExitConfig.detect_best_config_for_project(p1)).to eq(aec3)
    expect(Hmis::ProjectAutoExitConfig.for_project(p2)).to contain_exactly(aec2, aec4)
    expect(Hmis::ProjectAutoExitConfig.detect_best_config_for_project(p2)).to eq(aec4)
    ...
    expect(Hmis::ProjectAutoExitConfig.detect_best_config_for_project(p1)).to eq(aec5)
    expect(Hmis::ProjectAutoExitConfig.for_project(p2)).to contain_exactly(aec2, aec4)
    expect(Hmis::ProjectAutoExitConfig.detect_best_config_for_project(p2)).to eq(aec4)
  end
