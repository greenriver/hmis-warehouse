...
RSpec.describe Hmis::TableConfiguration, type: :model do
  ...
  describe 'validations' do
    subject { build(:hmis_table_configuration, table_key: 'ce_waitlist', data_source: data_source) }
    ...
    it 'validates uniqueness of table_key scoped to owner and data_source' do
      create(:hmis_table_configuration, table_key: 'ce_waitlist', owner: project, data_source: data_source)
      duplicate = build(:hmis_table_configuration, table_key: 'ce_waitlist', owner: project, data_source: data_source)
    ...
    it 'allows same table_key for different owners' do
      ...
      create(:hmis_table_configuration, table_key: 'ce_waitlist', owner: project, data_source: data_source)
      duplicate = build(:hmis_table_configuration, table_key: 'ce_waitlist', owner: project2, data_source: data_source)
    ...
    it 'allows same table_key for different data_sources' do
      ...
      create(:hmis_table_configuration_ce_waitlist, owner: project, data_source: data_source)
      duplicate = build(:hmis_table_configuration_ce_waitlist, owner: project, data_source: data_source2)
  ...
  describe 'column validation' do
    ...
    it 'validates valid columns' do
      config = build(:hmis_table_configuration_ce_waitlist, columns: valid_columns, data_source: data_source)
      expect(config).to be_valid
    end

    it 'rejects invalid column types' do
      config = build(:hmis_table_configuration_ce_waitlist, columns: invalid_columns, data_source: data_source)
    ...
    it 'accepts empty columns array' do
      config = build(:hmis_table_configuration_ce_waitlist, columns: [], data_source: data_source)
      expect(config).to be_valid
    end
  end
  ...
  describe 'filter validation' do
    ...
    it 'validates valid filters' do
      config = build(:hmis_table_configuration_ce_waitlist, filters: valid_filters, data_source: data_source)
      expect(config).to be_valid
    end

    it 'rejects invalid filter types' do
      config = build(:hmis_table_configuration_ce_waitlist, filters: invalid_filters, data_source: data_source)
      expect(config).not_to be_valid
    ...
    it 'accepts empty filters array' do
      config = build(:hmis_table_configuration_ce_waitlist, filters: [], data_source: data_source)
      expect(config).to be_valid
    end
  end
end
