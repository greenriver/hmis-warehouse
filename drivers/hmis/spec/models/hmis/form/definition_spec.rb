...
RSpec.describe Hmis::Form::Definition, type: :model do
  ...
  describe 'the latest_versions scope' do
    ...
    let!(:id1_retired1) { create(:hmis_form_definition, data_source: ds1, identifier: 'identifier_1', version: 0, status: 'retired') }
    let!(:id1_retired2) { create(:hmis_form_definition, data_source: ds1, identifier: 'identifier_1', version: 1, status: 'retired') }
    let!(:id1_published) { create(:hmis_form_definition, data_source: ds1, identifier: 'identifier_1', version: 2, status: 'published') }
    let!(:id1_draft) { create(:hmis_form_definition, data_source: ds1, identifier: 'identifier_1', version: 3, status: 'draft') }

    # id2 exists only in draft
    let!(:id2_draft) { create(:hmis_form_definition, data_source: ds1, identifier: 'identifier_2', version: 0, status: 'draft') }

    # id3 has 2 retired versions, no currently published or draft versions
    let!(:id3_retired1) { create(:hmis_form_definition, data_source: ds1, identifier: 'identifier_3', version: 0, status: 'retired') }
    let!(:id3_retired2) { create(:hmis_form_definition, data_source: ds1, identifier: 'identifier_3', version: 1, status: 'retired') }
  ...
  describe 'pick-list metadata extraction' do
    let(:choice_items) do
      [
        {
          'type' => 'CHOICE',
    ...
    let!(:definition) do
      create(:hmis_form_definition, data_source: ds1, role: :CUSTOM_ASSESSMENT, identifier: 'choice-form', version: 1, append_items: choice_items)
    end

    describe '#pick_list_metadata_by_key' do
      it 'extracts item type, inline options, and references for custom fields' do
        metadata = definition.pick_list_metadata_by_key

        expect(metadata['decision']).to match(
          item_type: 'CHOICE',
          pick_list_reference: nil,
          pick_list_options: [
            a_hash_including('code' => 'hmis_user_error', 'label' => 'HMIS user error'),
            a_hash_including('code' => 'client_declined', 'label' => 'Client declined'),
          ],
        )
        expect(metadata['prior_living']).to match(
          item_type: 'CHOICE',
          pick_list_reference: 'PRIOR_LIVING_SITUATION',
          pick_list_options: nil,
        )
        expect(metadata['notes'][:pick_list_options]).to be_nil
        expect(metadata['notes'][:pick_list_reference]).to be_nil
      end
    end
    ...
    describe '#pick_list_metadata_across_versions' do
      ...
      let!(:published_version) do
        create(:hmis_form_definition, data_source: ds1, role: :CUSTOM_ASSESSMENT, identifier: 'unioned-form', version: 2, status: :published, append_items: [
                 {
                   'type' => 'CHOICE',
                   'link_id' => 'decision',
                   'text' => 'Decision',
                   'mapping' => { 'custom_field_key' => 'decision' },
                   'pick_list_options' => [
                     # New code introduced in the latest version
