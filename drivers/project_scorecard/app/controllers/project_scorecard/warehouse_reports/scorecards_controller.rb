...
# @see docs/features/warehouse/project-scorecard.md
module ProjectScorecard::WarehouseReports
  class ScorecardsController < ApplicationController
    ...
    def link_to_apr(text, question)
      helpers.link_to_if(GrdaWarehouse::WarehouseReports::ReportDefinition.url_viewable_by?('hud_reports/aprs', current_user) && @report.apr_id.present?, text, result_hud_reports_apr_question_path(@report.apr_id || 0, question), target: :blank)
    end
    helper_method :link_to_apr

    def link_to_spm(text, measure)
      helpers.link_to_if(GrdaWarehouse::WarehouseReports::ReportDefinition.url_viewable_by?('hud_reports/spms', current_user) && @report.spm_id.present?, text, result_hud_reports_spm_measure_path(@report.spm_id || 0, measure), target: :blank)
    end
