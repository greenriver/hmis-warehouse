...
module AllNeighborsSystemDashboard
  class Enrollment < GrdaWarehouseBase
    ...
    scope :moved_in, -> do
      where.not(move_in_date: nil, project_type: HudUtilityCurrent.project_types_with_move_in_dates)
    end

    scope :moved_in_in_range, ->(range, filter:) do
      where(move_in_date: range, project_type: HudUtilityCurrent.project_types_with_move_in_dates, project_id: filter.effective_project_ids)
    end
