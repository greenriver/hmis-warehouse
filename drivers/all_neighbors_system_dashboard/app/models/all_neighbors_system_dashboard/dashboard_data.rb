...
module AllNeighborsSystemDashboard
  class DashboardData
    ...
    def demographic_race
      ...
      Hud.util.races(multi_racial: true).except(*ignored_races).values.sort + [Hud.util.races(multi_racial: true)['RaceNone']]
    end
    ...
    def demographic_gender
      Hud.util.genders.values - unknown_genders + ['Unknown Gender']
    end
    ...
    private def filter_for_type(scope, type)
      ...
      when 'Unsheltered', 'Unhoused Population'
        scope.where(project_type: Hud.util.project_type('Street Outreach', true), project_id: @report.filter.effective_project_ids).
          # Prioritize counting clients in diversion over by project type
          where.not(project_id: @report.filter.secondary_project_ids)
      when 'Sheltered'
        scope.where.not(project_type: Hud.util.project_type('Street Outreach', true), project_id: @report.filter.effective_project_ids).
          # Prioritize counting clients in diversion over by project type
          where.not(project_id: @report.filter.secondary_project_ids)
      ...
      when *Hud.util.gender_known_values
        scope.where(gender: type)
      when 'Unknown Gender'
        scope.where.not(gender: Hud.util.gender_known_values)
      when *Hud.util.races(multi_racial: false).values
