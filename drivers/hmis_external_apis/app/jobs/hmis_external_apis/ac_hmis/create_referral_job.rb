...
module HmisExternalApis::AcHmis
  class CreateReferralJob < BaseJob
    ...
    def gender_attributes_from_codes(codes)
      attributes = Hud.util.gender_id_to_field_name.invert.excluding(:GenderNone).map do |k, v|
    ...
    def race_attributes_from_codes(codes)
      attributes = Hud.util.race_id_to_field_name.invert.excluding(:RaceNone).map do |k, v|
        [k, codes.include?(v) ? 1 : 0]
      end.to_h
