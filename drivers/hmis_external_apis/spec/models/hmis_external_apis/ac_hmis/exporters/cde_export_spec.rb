...
RSpec.describe HmisExternalApis::AcHmis::Exporters::CdeExport, type: :model do
  ...
  let!(:cde1) { create :hmis_custom_data_element, data_element_definition: cded, owner: hud_service, data_source: ds, value_string: 'First value' }
  ...
  it 'makes a csv' do
    ...
    expect(result.first['DateCreated']).to eq(cde1.date_created.to_s)
    expect(result.first['DateUpdated']).to eq(cde1.date_updated.to_s)
  end
end
