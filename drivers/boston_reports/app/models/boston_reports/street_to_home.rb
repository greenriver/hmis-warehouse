  ...
  class StreetToHome
    ...
    def housed_by_cohort
      @housed_by_cohort ||= {}.tap do |data|
        active_months = stages[housed_string][:scope].pluck(housed_date_instance.column).compact.map(&:beginning_of_month).uniq.sort
    ...
    def matched_by_cohort
      @matched_by_cohort ||= {}.tap do |data|
        active_months = stages[matched_string][:scope].pluck(voucher_date_instance.column).compact.map { |d| Date.parse(d).beginning_of_month }.uniq.sort
        months = [active_months.first]
        month = active_months.first
        while month < active_months.last
          month += 1.months
