...
# r = HudLsa::Generators::Fy2027::Lsa.last
...
module HudLsa::Generators::Fy2027
  class Lsa < ::HudReports::ReportInstance
    ...
    has_one :summary_result, class_name: 'HudLsa::Fy2027::SummaryResult', foreign_key: :hud_report_instance_id
    belongs_to :export, class_name: 'GrdaWarehouse::HmisExport', optional: true

    scope :lsa, -> do
      where("(options->>'lsa_scope')::integer != ? OR options->>'lsa_scope' IS NULL", HudLsa::Fy2027::Report.available_lsa_scopes['HIC'])
    end
    ...
    def hic?
      options.with_indifferent_access[:lsa_scope].to_i == HudLsa::Fy2027::Report.available_lsa_scopes['HIC']
    end
    ...
    def run_lsa_queries
      ...
      load 'lib/rds_sql_server/lsa/fy2027/lsa_queries.rb'
    ...
    def fetch_summary_results
      load 'lib/rds_sql_server/lsa/fy2027/lsa_report_summary.rb'
    ...
    def populate_hmis_tables
      load 'lib/rds_sql_server/lsa/fy2027/hmis_sql_server.rb' # provides thin wrappers to all HMIS tables
    ...
    memoize private def extract_path
      ...
      source = Rails.root.join('drivers/hud_lsa/spec/fixtures/files/lsa/fy2027/sample_hmis_export/')
    ...
    # This reverses some export changes we made to maintain case sensitive matching with the 2027 spec
    ...
    def fetch_results
      load 'lib/rds_sql_server/lsa/fy2027/lsa_sql_server.rb'
    ...
    def self.archival_csv_config(report_instance)
      HudReportArchival.shared_archival_entries(report_instance, prefix: 'lsa').merge(
        lsa_summary_results_csv: {
          scope: -> { HudLsa::Fy2027::SummaryResult.where(hud_report_instance_id: report_instance.id) },
          filename: -> { "hud-lsa-fy2027-#{report_instance.id}-summary-results.csv" },
          delete_order: 2,
        },
      )
    end
