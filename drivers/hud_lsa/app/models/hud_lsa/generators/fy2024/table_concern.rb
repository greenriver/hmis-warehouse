...
module HudLsa::Generators::Fy2024::TableConcern
  ...
  def setup_hmis_table_structure
    ...
    load 'lib/rds_sql_server/lsa/fy2024/hmis_sql_server.rb'
  ...
  def setup_lsa_table_structure
    ::Rds.identifier = sql_server_identifier unless Rds.static_rds?
    ::Rds.database = sql_server_database
    load 'lib/rds_sql_server/lsa/fy2024/lsa_table_structure.rb'
  end
  ...
  def setup_lsa_table_indexes
    SqlServerBase.connection.execute <<~SQL
      #{File.read('lib/rds_sql_server/lsa/fy2024/sample_code/00 Totally Optional HMIS Table Indexes.sql')}
    SQL
    SqlServerBase.connection.execute(<<~SQL)
