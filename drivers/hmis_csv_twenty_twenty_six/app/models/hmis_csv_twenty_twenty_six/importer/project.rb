...
module HmisCsvTwentyTwentySix::Importer
  class Project < GrdaWarehouse::Hud::Base
    ...
    Hud.util('2026').residential_project_type_numbers_by_code.each do |k, v|
    ...
    scope :residential, -> do
      where(
        arel_table[:ProjectType].in(Hud.util('2026').residential_project_type_ids - [13]).
        or(
    ...
    def self.hmis_validations
      {
        ...
        ContinuumProject: [
          ...
          },
          {
            ...
            arguments: { valid_options: Hud.util('2026').yes_no_missing_options.keys.map(&:to_s).freeze },
          },
        ],
        ProjectType: [
          {
            class: HmisCsvImporter::HmisCsvValidation::InclusionInSet,
            arguments: { valid_options: Hud.util('2026').project_types.keys.map(&:to_s).freeze },
          },
        ...
        HousingType: [
          {
            class: HmisCsvImporter::HmisCsvValidation::InclusionInSet,
            arguments: { valid_options: Hud.util('2026').housing_types.keys.map(&:to_s).freeze },
          },
        ],
        ...
        TargetPopulation: [
          {
            class: HmisCsvImporter::HmisCsvValidation::InclusionInSet,
            arguments: { valid_options: Hud.util('2026').target_populations.keys.map(&:to_s).freeze },
          },
        ],
        HOPWAMedAssistedLivingFac: [
          {
            class: HmisCsvImporter::HmisCsvValidation::InclusionInSet,
            arguments: { valid_options: Hud.util('2026').hopwa_med_assisted_living_facs.keys.map(&:to_s).freeze },
          },
        ],
      }
    end
