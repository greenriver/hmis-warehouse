...
module HmisCsvTwentyTwentySix::Importer::ImportConcern
  ...
  included do
    ...
    def self.create_columns
      (hmis_structure.keys + [:data_source_id]).map(&:to_s).freeze
    end
    ...
    def self.hmis_keys
      @hmis_keys ||= hmis_structure.keys
    end

    def hmis_data
      slice(*self.class.hmis_keys) # self.class.hmis_keys is used to avoid realloc/calc'ing these static keys in tight import loops
    end
    ...
    def self.run_complex_validations!(importer_log, filename)
      ...
      complex_validations.each do |check|
        Rails.logger.info { "Running #{check[:class]} for #{self}" }
