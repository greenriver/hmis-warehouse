...
RSpec.shared_context '2026 single-project tests', shared_context: :metadata do
  describe 'when exporting projects' do
    ...
    it 'creates one CSV file' do
      expect(File.exist?(ExportHelper2026.csv_file_path(ExportHelper2026.project_class))).to be true
    end
    it 'adds one row to the project CSV file' do
      csv_projects = CSV.read(ExportHelper2026.csv_file_path(ExportHelper2026.project_class), headers: true)
      expect(csv_projects.count).to eq 1
    end
    it 'project from CSV file should have the same name as the first project' do
      csv_projects = CSV.read(ExportHelper2026.csv_file_path(ExportHelper2026.project_class), headers: true)
      expect(csv_projects.first['ProjectName']).to eq ExportHelper2026.projects.first.ProjectName
    end
    it 'ProjectID from CSV file match the id of first project' do
      csv_projects = CSV.read(ExportHelper2026.csv_file_path(ExportHelper2026.project_class), headers: true)
      expect(csv_projects.first['ProjectID']).to eq ExportHelper2026.projects.first.id.to_s
    end
  end
  ...
  ExportHelper2026.project_classes.each do |klass|
    describe "when exporting #{klass}" do
      it "creates one #{klass.hud_csv_file_name} CSV file" do
        expect(File.exist?(ExportHelper2026.csv_file_path(klass))).to be true
      end
      it "adds one row to the #{klass.hud_csv_file_name} CSV file" do
        csv = CSV.read(ExportHelper2026.csv_file_path(klass), headers: true)
        expect(csv.count).to eq 1
      end
      ...
      if klass.hmis_class.column_names.include?('ProjectID')
        it 'ProjectID from CSV file match the id of first project' do
          csv = CSV.read(ExportHelper2026.csv_file_path(klass), headers: true)
          expect(csv.first['ProjectID']).to eq ExportHelper2026.projects.first.id.to_s
        end
      end
    end
  end
...
RSpec.configure do |rspec|
  rspec.include_context '2026 single-project tests', include_shared: true
end
