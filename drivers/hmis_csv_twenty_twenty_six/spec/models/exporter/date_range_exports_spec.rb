...
RSpec.describe HmisCsvTwentyTwentySix::Exporter::Base, type: :model do
  ...
  before(:all) do
    ...
    ExportHelper2026.enrollments.sort_by(&:id).last(2).each { |m| m.update(EntryDate: 2.days.ago) }
    # Move 3 exits into the range
    ExportHelper2026.exits.sort_by(&:id).first(3).each { |m| m.update(ExitDate: 2.weeks.ago) }
    # Move 3 assessments into range
    ExportHelper2026.assessments.sort_by(&:id).first(3).each { |m| m.update(AssessmentDate: 2.weeks.ago) }
