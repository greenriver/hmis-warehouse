...
RSpec.shared_context '2026 multi-enrollment tests', shared_context: :metadata do
  def involved_projects
    GrdaWarehouse::Hud::Project.where(id: @involved_project_ids || ExportHelper2026.projects.map(&:id))
  end
  ...
  describe "When exporting enrollment related items for #{project_test_type}" do
    ...
    describe 'when exporting clients' do
      ...
      it 'creates one CSV file' do
        expect(File.exist?(ExportHelper2026.csv_file_path(ExportHelper2026.client_class))).to be true
      end
    ...
    ExportHelper2026.enrollment_related_items.each do |items, klass|
      describe "when exporting #{items}" do
        ...
        it "hud keys in #{klass.hud_csv_file_name} CSV should match ids of first three items in list" do
          ...
          source_ids = ExportHelper2026.send(items).select do |m|
            ...
            map(&:to_s)
          ExportHelper2026.send(items).select do |m|
            involved_enrollment_project_entry_ids.include? m.EnrollmentID
...
RSpec.configure do |rspec|
  rspec.include_context '2026 multi-enrollment tests', include_shared: true
end
