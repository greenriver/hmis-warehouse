...
RSpec.shared_context '2026 single-enrollment tests', shared_context: :metadata do
  describe 'when exporting enrollments' do
    ...
    it 'EnrollmentID from CSV file match the id of first enrollment' do
      csv = CSV.read(ExportHelper2026.csv_file_path(ExportHelper2026.enrollment_class), headers: true)
      expect(csv.first['EnrollmentID']).to eq ExportHelper2026.enrollments.first.id.to_s
    end
  ...
  ExportHelper2026.enrollment_related_items.each do |items, klass|
    describe "when exporting #{items}" do
      ...
      it 'hud key in CSV should match id of first item in list' do
        csv = CSV.read(ExportHelper2026.csv_file_path(klass), headers: true)
        expect(csv.first[klass.hmis_class.hud_key.to_s]).to eq ExportHelper2026.send(items).first.id.to_s
      end
...
RSpec.configure do |rspec|
  rspec.include_context '2026 single-enrollment tests', include_shared: true
end
