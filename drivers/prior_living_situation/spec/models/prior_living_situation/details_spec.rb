...
RSpec.describe PriorLivingSituation::Details, type: :model do
  let(:user) { create(:user) }
  let(:filter) { ::Filters::FilterBase.new(user: user, start: Date.current.beginning_of_year, end: Date.current.end_of_year) }
  let(:report) { PriorLivingSituation::PriorLivingSituationReport.new(filter) }
  ...
  let(:restricted_values) do
    { 'Client ID' => restricted_destination_client.id, 'First Name' => 'Restricted', 'Last Name' => 'Client', 'DOB' => Date.new(1990, 1, 1), 'Sex' => 1 }
  end
  let(:open_values) do
    { 'Client ID' => open_destination_client.id, 'First Name' => 'Open', 'Last Name' => 'Doe', 'DOB' => Date.new(1985, 6, 15), 'Sex' => 0 }
  end
