...
module PriorLivingSituation
  class PriorLivingSituationReport
    ...
    def data_for_living_situations
      @data_for_living_situations ||= begin
        ...
        report_scope.joins(:enrollment, project: :project_cocs).
          ...
          pluck(
            ...
            living_situation = HudHelper.util.situation_type(living_situation_id).gsub('Housing', '').strip
            ...
            # data[:by_coc][coc_code][:situations_length] ||= living_situation_buckets.product(HudHelper.util.residence_prior_length_of_stays_brief.values.uniq).map { |b| [b, Set.new] }.to_h
            data[:by_coc][coc_code][:situations_length] ||= living_situation_buckets.map { |b| [b, {}] }.to_h
            living_situation_buckets.each do |b|
              HudHelper.util.residence_prior_length_of_stays_brief.values.uniq.each do |l|
                data[:by_coc][coc_code][:situations_length][b][l] ||= Set.new
              end
            end

            data[:by_coc][coc_code][:situations][living_situation] << client_id
            data[:by_coc][coc_code][:situations_length][living_situation][HudHelper.util.residence_prior_length_of_stay_brief(length_of_stay) || ''] << client_id
          end
        data
      end
      ...
      # columns:
      #   'location' ()
      #   'length of stay' HudHelper.util.residence_prior_length_of_stay_brief
    end
