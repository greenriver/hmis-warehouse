...
module EccoviaData
  class CaseManager < GrdaWarehouseBase
    ...
    def self.fetch_updated(data_source_id:, credentials:)
      ...
      ids = warehouse_enrollment_ids(data_source_id: data_source_id, credentials: credentials)
      query = "crql?q=select ClientID, EnrollID, ClntCaseID from Enrollment where EnrollID in (#{quote(ids)}) and UpdatedDate > '#{since.to_s(:db)}'"
      credentials.get_all_in_batches(query) do |enrollments|
        ...
        batch = enrollments.map do |enrollment|
          ...
          new(
            ...
            first_name: assignment['CaseMgrName']&.split(' ')&.first,
            last_name: assignment['CaseMgrName']&.split(' ')&.last,
            email: user['Email'],
    ...
    def self.warehouse_enrollment_ids(data_source_id:, credentials:)
      ...
      if Rails.env.development?
        ...
        GrdaWarehouse::Hud::Enrollment.where(data_source_id: data_source_id).with_project_type(14).pluck(:EnrollmentID)
      end
    end
