...
module SystemPathways::ChartBase
  ...
  included do
    ...
    def long_project_type(project_type_brief)
      number = HudUtilityCurrent.project_type_number(project_type_brief)
      HudUtilityCurrent.project_type(number)
    end
    ...
    def sanitized_node(node)
      ...
      available = report.class.available_project_types.map do |p_type|
        HudUtilityCurrent.project_type_brief(p_type)
    ...
    def node_clients(node)
      if node == 'Served by Homeless System'
        ...
        to_project_type = HudUtilityCurrent.project_type_number(ph_projects.invert[node])
        ...
        to_project_type = HudUtilityCurrent.project_type_number(node)
        SystemPathways::Enrollment.where(project_type: to_project_type).
          joins(:client).
          merge(filtered_clients).
          distinct
    ...
    private def races
      @races ||= HudUtilityCurrent.races(multi_racial: true)
    end

    private def ethnicities
      @ethnicities ||= HudUtilityCurrent.ethnicities
    end

    private def race_ethnicity_combinations
      @race_ethnicity_combinations ||= HudUtilityCurrent.race_ethnicity_combinations
    end

    private def veteran_statuses
      @veteran_statuses ||= HudUtilityCurrent.no_yes_reasons_for_missing_data_options
    end
    ...
    private def disabling_conditions
      @disabling_conditions ||= HudUtilityCurrent.no_yes_reasons_for_missing_data_options
    end
    ...
    private def race_columns
      @report.race_col_lookup.map { |k, hud_k| [k, HudUtilityCurrent.race(hud_k)] }.to_h
    end
