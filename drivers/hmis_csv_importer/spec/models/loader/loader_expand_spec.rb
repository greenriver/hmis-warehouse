...
RSpec.describe HmisCsvImporter::Loader::Loader, type: :model do
  describe '#expand' do
    ...
    # #expand writes to the local path, which the loader never assigns itself --
    # callers set it.
    let(:loader) do
      # The initializer needs a directory, and the loader reads Export.csv from
      # it to calculate the current version.
