  ...
  describe 'When importing clients' do
    before(:all) do
      ...
      import_hmis_csv_fixture(
        'drivers/hmis_csv_importer/spec/fixtures/files/twenty_twenty/client_processing',
        version: 'AutoMigrate',
        run_jobs: false,
      )
    end
    ...
    describe 'when re-importing again after nulling source_hash' do
      ...
      describe 'when re-importing and the source hash doesn\'t match' do
        ...
        describe 'when re-importing with changed enrollment' do
          before(:all) do
            import_hmis_csv_fixture(
              'drivers/hmis_csv_importer/spec/fixtures/files/twenty_twenty/client_processing_2',
              version: 'AutoMigrate',
              run_jobs: false,
            )
          end
