...
RSpec.describe 'Append Organization IDs', type: :model do
  ...
  describe 'with cleanup' do
    ...
    it 'has Organization ID appended' do
      expect(GrdaWarehouse::Hud::Organization.first.OrganizationName).to eq('Test Org (ORG-ID)')
    end
  end
  ...
  def setup(with_cleanup:)
    ...
    @data_source = if with_cleanup
      create(:importer_append_organization_ids)
    else
