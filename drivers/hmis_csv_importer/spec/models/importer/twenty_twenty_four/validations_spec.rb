...
RSpec.describe 'Validate import files', type: :model do
  before(:all) do
    ...
    import_hmis_csv_fixture(
      'drivers/hmis_csv_importer/spec/fixtures/files/twenty_twenty_four/validation_files',
  ...
  it 'includes expected client validations' do
    aggregate_failures 'validating' do
      expect(HmisCsvImporter::HmisCsvValidation::InclusionInSet.where("source_type LIKE '%Client'").count).to eq(3)
  ...
  it 'includes expected projects validations' do
    aggregate_failures 'validating' do
      expect(HmisCsvImporter::HmisCsvValidation::InclusionInSet.where("source_type LIKE '%Project'").count).to eq(5)
      expect(HmisCsvImporter::HmisCsvValidation::NonBlankValidation.where("source_type LIKE '%Project'").count).to eq(2)
    end
  end
