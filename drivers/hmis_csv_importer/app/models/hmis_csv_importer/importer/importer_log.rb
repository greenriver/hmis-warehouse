...
module HmisCsvImporter::Importer
  class ImporterLog < GrdaWarehouseBase
    ...
    # for debugging, parse the logs generated by HmisCsvImporter::Importer::Importer#with_sql_log
    def debug_phases(show_sql: false)
      return if phase_metrics.blank?

      log_data = phase_metrics
      # log_data = JSON.parse(File.read(Rails.root.join('test.json')))

      # Sort phases by duration (if available)
      sorted_phases = log_data.sort_by do |_phase_name, phase_data|
        # -(phase_data['duration'] || 0).to_f
        DateTime.parse(phase_data['started_at'])
      end
      ...
      sorted_phases.each do |phase_name, phase_data|
        ...
        formatted_output += "  started_at: #{phase_data['started_at']}\n"
        ...
        phase_data.each do |key, value|
          ...
          sorted_queries.each_with_index do |query_info, index|
            ...
            formatted_output += "       #{query_info['duration']} seconds:\n"

            next unless show_sql && query_info['compressed_query']
