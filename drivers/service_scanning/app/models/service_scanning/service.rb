...
module ServiceScanning
  class Service < GrdaWarehouseBase
    ...
    def self.project_options_for_select(user:)
      ...
      project_scope = project_scope.merge(::GrdaWarehouse::Hud::Project.non_confidential) unless user&.can_view_confidential_project_names?
      project_scope.
        joins(:organization).
        ...
        options[org] ||= []
        options[org] << ["#{project_name} (#{HudUtilityCurrent.project_type_brief(project_type)})", id]
      end
      options
    end
