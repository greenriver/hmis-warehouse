...
module HudDataQualityReport
  class BaseController < ::HudReports::BaseController
    ...
    private def path_for_running_all_questions
      running_all_questions_hud_reports_past_dqs_path(link_params.except('action', 'controller'))
    end

    private def path_for_running_question
      running_hud_reports_past_dqs_path(link_params.except('action', 'controller'))
    end
