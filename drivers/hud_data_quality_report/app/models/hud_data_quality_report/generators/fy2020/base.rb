...
module HudDataQualityReport::Generators::Fy2020
  class Base < ::HudReports::QuestionBase
    ...
    private def household_veterans_chronically_homeless?(dq_client)
      adults = household_adults(dq_client)
      veterans = household_veterans(adults)
      household_chronically_homeless_clients(veterans).any?
    end

    private def household_veterans_non_chronically_homeless?(dq_client)
      adults = household_adults(dq_client)
      veterans = household_veterans(adults)
    ...
    private def household_adults(dq_client)
      return [] unless dq_client.household_members

      date = [dq_client.first_date_in_program, @report.start_date].max
      dq_client.household_members.select do |member|
    ...
    private def all_household_adults_veterans?(dq_client)
      household_adults(dq_client).all? do |member|
        member['veteran_status'] == 1
      end
    end

    private def all_household_adults_non_veterans?(dq_client)
      household_adults(dq_client).all? do |member|
    ...
    private def household_adults_refused_veterans(dq_client)
      household_adults(dq_client).select do |member|
        member['veteran_status'].in?([8, 9])
      end
    end

    private def household_adults_missing_veterans(dq_client)
      household_adults(dq_client).select do |member|
    ...
    private def only_youth?(dq_client)
      youth_household_members(dq_client).count == dq_client.household_members.count
    end

    private def youth_household_members(dq_client)
      return [] unless dq_client.household_members
    ...
    private def youth_child_members(dq_client)
      youth_household_members(dq_client).select do |member|
        member['relationship_to_hoh'] == 2
      end
    end

    private def youth_children?(dq_client)
      youth_child_members(dq_client).any?
    end

    private def youth_child_source_client_ids(dq_client)
      youth_child_members(dq_client).map { |member| member['source_client_id'] }
    end

    private def adult_source_client_ids(dq_client)
      household_adults(dq_client).map { |member| member['source_client_id'] }
    end

    private def youth_parent?(dq_client)
      dq_client.head_of_household && only_youth?(dq_client) && youth_children?(dq_client)
    end
    ...
    private def earned_amount(dq_client, suffix)
      dq_client["income_sources_at_#{suffix}"]['EarnedAmount']
    end

    private def other_amount(dq_client, suffix)
      total_amount = total_amount(dq_client, suffix)
      return 0 unless total_amount.present? && total_amount.positive?

      earned = earned_amount(dq_client, suffix).presence || 0
      total_amount.to_i - earned.to_i
    end

    private def total_amount(dq_client, suffix)
      dq_client["income_total_at_#{suffix}"]
    end

    # We have earned income if we said we had earned income and the amount is positive
    private def earned_income?(dq_client, suffix)
      return false unless dq_client["income_sources_at_#{suffix}"]['Earned'] == 1

      earned_amt = earned_amount(dq_client, suffix)
    ...
    private def other_income?(dq_client, suffix)
      total_amount = total_amount(dq_client, suffix)
      return false unless total_amount.present? && total_amount.positive?

      total_amount != earned_amount(dq_client, suffix).to_i
    end

    private def total_income?(dq_client, suffix)
      total_amount = total_amount(dq_client, suffix)
      total_amount.present? && total_amount.positive?
    end

    private def income_for_category?(dq_client, category:, suffix:)
      case category
      when :earned
        earned_income?(dq_client, suffix)
      when :other
        other_income?(dq_client, suffix)
      when :total
        total_income?(dq_client, suffix)
      end
    end

    private def both_income_types?(dq_client, suffix)
      earned_income?(dq_client, suffix) && other_income?(dq_client, suffix)
    end

    private def no_income?(dq_client, suffix)
      [
        earned_income?(dq_client, suffix),
        other_income?(dq_client, suffix),
      ].none?
    ...
    private def income_change(dq_client, category:, initial:, subsequent:)
      case category
      when :total
        initial_amount = dq_client["income_total_at_#{initial}"]
        subsequent_amount = dq_client["income_total_at_#{subsequent}"]
      when :earned
        initial_amount = earned_amount(dq_client, initial)
        subsequent_amount = earned_amount(dq_client, subsequent)
      when :other
        initial_amount = other_amount(dq_client, initial)
        subsequent_amount = other_amount(dq_client, subsequent)
      end
