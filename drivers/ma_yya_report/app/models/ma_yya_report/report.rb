    ...
    private def calculators # rubocop:disable Metrics/AbcSize
...

      {
        ...
        F1b: a_t[:followup_previous_period].eq(false).
          and(Arel.sql(
                json_contains(:subsequent_current_living_situations, Hud.util.permanent_situations(as: :current) + Hud.util.temporary_situations(as: :current) + Hud.util.institutional_situations(as: :current) - [302]), # Excludes 302: Transitional housing for homeless persons (including homeless youth)
              )),
        ...
        F2c: a_t[:currently_homeless].eq(true).
          ...
          and(Arel.sql(json_contains(:subsequent_current_living_situations, Hud.util.permanent_situations(as: :current)))),
