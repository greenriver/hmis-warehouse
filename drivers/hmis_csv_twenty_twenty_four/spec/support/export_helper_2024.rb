...
module ExportHelper2024
  class << self
    ...
    def csv_file_path(klass, exporter: @exporter)
      return File.join(exporter.file_path, 'Export.csv') if klass == HmisCsvTwentyTwentyFour::Exporter::Export

      return File.join(exporter.file_path, exporter.hmis_class_for(klass).hud_csv_file_name(version: '2024')) unless klass.is_a?(String)

      # If klass is a string like 'CEParticipation', convert it to the corresponding exporter class
      klass = "HmisCsvTwentyTwentyFour::Exporter::#{normalize_class_name(klass)}".constantize
