...
RSpec.shared_context '2024 multi-project tests', shared_context: :metadata do
  describe "When exporting project related items for #{project_test_type}" do
    ...
    ExportHelper2024.project_related_items.each do |items, klass|
      describe "when exporting #{klass}" do
        ...
        if klass.hmis_class.column_names.include?('ProjectID')
          it 'hud keys in CSV should match source data' do
            ...
            source_keys = ExportHelper2024.send(items).select do |m|
...
RSpec.configure do |rspec|
  rspec.include_context '2024 multi-project tests', include_shared: true
end
