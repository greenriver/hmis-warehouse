...
RSpec.shared_context '2024 single-enrollment tests', shared_context: :metadata do
  describe 'when exporting enrollments' do
    ...
    it 'EnrollmentID from CSV file match the id of first enrollment' do
      csv = CSV.read(ExportHelper2024.csv_file_path(ExportHelper2024.enrollment_class), headers: true)
      expect(csv.first['EnrollmentID']).to eq ExportHelper2024.enrollments.first.id.to_s
    end
  ...
  describe 'when exporting clients' do
    ...
    it 'PersonalID from CSV file match the id of first client' do
      csv = CSV.read(ExportHelper2024.csv_file_path(ExportHelper2024.client_class), headers: true)
      expect(csv.first['PersonalID']).to eq ExportHelper2024.clients.first.destination_client.id.to_s
    end
  end
  ExportHelper2024.enrollment_related_items.each do |items, klass|
    describe "when exporting #{items}" do
      ...
      it 'hud key in CSV should match id of first item in list' do
        csv = CSV.read(ExportHelper2024.csv_file_path(klass), headers: true)
        expect(csv.first[klass.hmis_class.hud_key.to_s]).to eq ExportHelper2024.send(items).first.id.to_s
      end
