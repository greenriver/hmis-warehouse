...
RSpec.describe HmisCsvTwentyTwentyFour::Exporter::Base, type: :model do
  describe 'date range filtering' do
    before(:all) do
      ...
      @involved_project_ids = ExportHelper2024.projects.map(&:id)

      # move some data outside the date range
      @old_ce_participations = ExportHelper2024.ce_participations.first(4)
      ...
      @old_hmis_participations = ExportHelper2024.hmis_participations.first(4)
      ...
      @old_funders = ExportHelper2024.funders.first(4)
      ...
      @old_projects = ExportHelper2024.projects.first(4)
      ...
      @old_inventories = ExportHelper2024.inventories.first(4)
    ...
    context 'when enforce_project_date_scope is true' do
      ...
      it 'only includes CE Participation records within the specified date range' do
        csv = CSV.read(ExportHelper2024.csv_file_path('CEParticipation'), headers: true)
      ...
      it 'only includes HMIS Participation records within the specified date range' do
        csv = CSV.read(ExportHelper2024.csv_file_path('HMISParticipation'), headers: true)
        expect(csv.count).to eq(1)
      ...
      it 'only includes Funder records within the specified date range' do
        csv = CSV.read(ExportHelper2024.csv_file_path('Funder'), headers: true)
        expect(csv.count).to eq(1)
        expect(csv.first['FunderID']).to eq(@valid_funder.id.to_s)
      end

      it 'only includes Project records within the specified date range' do
        csv = CSV.read(ExportHelper2024.csv_file_path('Project'), headers: true)
        expect(csv.count).to eq(1)
      ...
      it 'only includes Inventory records within the specified date range' do
        csv = CSV.read(ExportHelper2024.csv_file_path('Inventory'), headers: true)
        expect(csv.count).to eq(1)
