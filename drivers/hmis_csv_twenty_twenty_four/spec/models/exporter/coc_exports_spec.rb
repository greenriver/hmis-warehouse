...
RSpec.describe HmisCsvTwentyTwentyFour::Exporter::Base, type: :model do
  ...
  before(:all) do
    ...
    en_1 = ExportHelper2024.enrollments.min_by(&:id)
    en_1.update(EnrollmentCoC: nil)
    en_2 = ExportHelper2024.enrollments.max_by(&:id)
    en_2.update(EnrollmentCoC: 'XX-501')

    # Move two unrelated project CoCs out of the range
    p_cocs = ExportHelper2024.project_cocs.reject { |pc| pc.ProjectID.in?([en_1.ProjectID, en_1.ProjectID]) }.first(2)
    ...
    @multi_coc_project = create :hud_project, data_source_id: ExportHelper2024.data_source.id
    create_list :hud_project_coc, 2, data_source_id: ExportHelper2024.data_source.id, CoCCode: 'XX-501', ProjectID: @multi_coc_project.ProjectID
    create_list :hud_enrollment, 3, data_source_id: ExportHelper2024.data_source.id, EntryDate: 2.weeks.ago, PreferredLanguageDifferent: 'a' * 10, EnrollmentCoC: 'XX-555', ProjectID: @multi_coc_project.ProjectID
    ...
    @project_with_bad_enrollment_coc = create :hud_project, data_source_id: ExportHelper2024.data_source.id
    create :hud_project_coc, data_source_id: ExportHelper2024.data_source.id, CoCCode: 'XX-501', ProjectID: @project_with_bad_enrollment_coc.ProjectID
    create_list :hud_enrollment, 2, data_source_id: ExportHelper2024.data_source.id, EntryDate: 2.weeks.ago, PreferredLanguageDifferent: 'a' * 10, EnrollmentCoC: nil, ProjectID: @project_with_bad_enrollment_coc.ProjectID
    ...
    @involved_project_ids = ExportHelper2024.projects.map(&:id) + [@multi_coc_project.id, @project_with_bad_enrollment_coc.id]
  ...
  describe 'Exporting for one CoC' do
    ...
    it 'EnrollmentIDs from CSV file match the ids of the enrollments in the CoC, are invalid, or are blank' do
      ...
      source_ids = involved_enrollments.select do |en|
        (en.EnrollmentCoC == @coc_code || en.EnrollmentCoC.blank? || ! Hud.util('2024').valid_coc?(en.EnrollmentCoC)) && en.project.project_cocs.pluck(:CoCCode).include?(@coc_code)
    ...
    it 'adds expected rows to the project CSV file' do
      csv = CSV.read(ExportHelper2024.csv_file_path(ExportHelper2024.project_class), headers: true)
      expect(csv.count).to eq 4
    end

    it 'creates one CSV file' do
      expect(File.exist?(ExportHelper2024.csv_file_path(ExportHelper2024.project_coc_class))).to be true
    end
