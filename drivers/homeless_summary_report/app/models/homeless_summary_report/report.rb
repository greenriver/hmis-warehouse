...
module HomelessSummaryReport
  class Report < SimpleReports::ReportInstance
              ...
              destination_buckets.each.with_index do |(destination_name, ids), i|
                ...
                unless ids == [0]
                  ...
                  ids.each do |d_id|
                    ...
                    details << "#{HudHelper.util.destination(d_id)}: #{count}" if count&.positive?
                  end
                end
    ...
    def measures
      {
        ...
        'Measure 7' => {
          fields: m7_fields,
          headers: destination_buckets.keys + ::HudHelper.util.valid_destinations.map { |id, d| "#{d} (#{id})" },
    ...
    private def destination_buckets
      {
        'Client Count' => [],
        'Homeless Destinations' => ::HudHelper.util.homeless_destinations,
        'Permanent Destinations' => HudHelper.util.permanent_destinations,
        'Temporary Destinations' => HudHelper.util.temporary_destinations,
        'Institutional Destinations' => HudHelper.util.institutional_destinations,
        'Unknown, doesn\'t know, refused, or not collected' => HudHelper.util.other_destinations,
        'Remained housed' => [0], # include those who remained housed for 7b2
      }.freeze
    end

    def destinations
      @destinations ||= destination_buckets.values.select(&:present?) + HudHelper.util.valid_destinations.keys
    end
