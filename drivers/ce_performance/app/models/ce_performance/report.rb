...
module CePerformance
  class Report < SimpleReports::ReportInstance
    ...
    HOUSING_REFERRAL_EVENTS = HudHelper.util.ce_events_referrals_to_housing.freeze
    ...
    def clients_title(sub_population_title: nil, vispdat_range: nil, vispdat_type: nil, event_type: nil)
      ...
      return "Event Type: #{::HudHelper.util.event(event_type)}" if event_type.present?
    ...
    private def any_cls_literally_homeless?(ce_apr_client)
      ce_apr_client.hud_report_apr_living_situations.any? do |m|
        m.living_situation.in?(::HudHelper.util.homeless_situations(as: :current))
      end
    end
