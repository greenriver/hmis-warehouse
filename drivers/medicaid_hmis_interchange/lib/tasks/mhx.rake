...
task medicaid_id_query: [:environment, 'log:info_to_stdout'] do
  if MedicaidHmisInterchange::MedicaidIdLookupJob.configured?
    ...
    homeless_clients.where.not(
      ...
      MedicaidHmisInterchange::MedicaidIdLookupJob.set(priority: BaseJob::CLEANUP_CACHE_REFRESH_PRIORITY_13).perform_later(batch)
    end
  end
end
