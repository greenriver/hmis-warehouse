...
RSpec.describe HmisCsvTwentyTwentyTwo::Exporter::Base, type: :model do
  describe 'without overrides' do
    before(:all) do
      ...
      @enrollment_without_enrollment_coc = create :hud_enrollment, data_source_id: @data_source.id, EntryDate: 3.weeks.ago, PersonalID: @enrollments.first.PersonalID, ProjectID: @enrollments.first.ProjectID
      @enrollment_with_invalid_enrollment_coc = create :hud_enrollment, data_source_id: @data_source.id, EntryDate: 3.weeks.ago, PersonalID: @enrollments.second.PersonalID, ProjectID: @enrollments.first.ProjectID
      @enrollment_coc_with_invalid_enrollment_coc = create :hud_enrollment_coc, data_source_id: @data_source.id, InformationDate: 1.week.ago, PersonalID: @enrollments.second.PersonalID, EnrollmentID: @enrollments.second.EnrollmentID, CoCCode: 'WRONG'
  ...
  describe 'with a ProjectCoC override' do
    ...
    it 'ignores Enrollments where EnrollmentCoC CoC Code is in a valid CoC, but is not in selected CoC' do
      csv = CSV.read(File.join(@exporter.file_path, 'Enrollment.csv'), headers: true)
      expect(csv.count).to eq 2
    end
