        ...
        quarter_dates.each do |date|
          data = with_service_in_quarter(report_scope, date, population).
            ...
            map do |gender_id, count|
              ...
              [
                ::HudUtilityCurrent.gender(gender_id) || 'Unknown',
                count,
              ]
            end.to_h
          data['Unknown'] ||= 0
    ...
    private def race_chart(population)
      {}.tap do |charts|
        quarter_dates.each do |date|
          ...
          census_data = {}
          # Add census info
          ::HudUtilityCurrent.races(multi_racial: true).except('HispanicLatinaeo', 'MidEastNAfrican').each do |race_code, label|
            census_data[label] = 0
            data[::HudUtilityCurrent.race(race_code, multi_racial: true)] ||= Set.new
            year = date.year
          ...
          with_service_in_quarter(report_scope, date, population).
            ...
            find_each do |enrollment|
              ...
              next unless data.key?(::HudUtilityCurrent.race(race, multi_racial: true))

              data[::HudUtilityCurrent.race(race, multi_racial: true)] << client.id unless client_ids.include?(client.id)
