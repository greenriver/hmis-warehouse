...
module HudHic::Fy2022
  class Inventory < ::GrdaWarehouseBase
    ...
    def self.from_attributes_for_hic(inventory)
      new(inventory.for_export.attributes.slice(*hmis_configuration(version: '2022').keys.map(&:to_s)))
    end

    # Hide ID, move client_id, and name to the front
    def self.detail_headers
      special = ['InventoryID', 'ProjectID']
