...
module HudPathReport
  class BaseController < ::HudReports::BaseController
    ...
    private def filter
      ...
      #raise @filter.method(:apply).source_location.inspect
      @filter.apply(GrdaWarehouse::ServiceHistoryEnrollment.all)
    ...
    private def path_for_running_all_questions
      running_all_questions_hud_reports_paths_path({ skip_trackable: true }.merge(link_params.except('action', 'controller')))
    end

    private def path_for_running_question
      running_hud_reports_paths_path({ skip_trackable: true }.merge(link_params.except('action', 'controller')))
    end
