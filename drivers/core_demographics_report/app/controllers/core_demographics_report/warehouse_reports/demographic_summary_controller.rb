  ...
  class DemographicSummaryController < ApplicationController
    ...
    def index
      ...
      respond_to do |format|
        format.html {}
        format.xlsx do
          filename = "#{Translation.translate('Demographic Summary')} - #{Time.current.to_fs(:db)}.xlsx"
    ...
    def details
      ...
      respond_to do |format|
        format.html {}
        format.xlsx do
          filename = "#{Translation.translate('Demographic Summary')} Support for #{@report.support_title(@key).gsub(',', '')} - #{Time.current.to_fs(:db)}.xlsx"
          headers['Content-Disposition'] = "attachment; filename=#{filename}"
        end
      end
    end
