...
CREATE SEQUENCE public.agencies_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
CREATE SEQUENCE public.idp_service_configs_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
CREATE SEQUENCE public.letsencrypt_plugin_challenges_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
CREATE SEQUENCE public.letsencrypt_plugin_settings_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
CREATE VIEW public.puma_scaling_login_demand AS WITH current_context AS (
    SELECT (now() AT TIME ZONE 'UTC'::text) AS "current_time",
        ...
        (
            EXTRACT(
                dow
                FROM (now() AT TIME ZONE 'UTC'::text)
            )
...
demand_window AS (
    SELECT CURRENT_TIME AS "current_time",
        ...
        (
            current_context.current_hour - '01:00:00'::interval
        ) AS window_start,
        (
            current_context.current_hour + '02:00:00'::interval
        ) AS window_end,
        (
            (
                current_context.current_hour - '01:00:00'::interval
            ) - current_context.current_day
        ) AS window_start_offset,
        (
            (
                current_context.current_hour + '02:00:00'::interval
            ) - current_context.current_day
        ) AS window_end_offset
    FROM current_context
),
historical_windows AS (
    SELECT (series_day.series_day)::date AS window_day,
        (
            series_day.series_day + demand_window.window_start_offset
        ) AS window_start,
        (
            series_day.series_day + demand_window.window_end_offset
        ) AS window_end
    FROM (
            demand_window
            CROSS JOIN LATERAL generate_series(
                (demand_window.current_day - '30 days'::interval),
                (demand_window.current_day - '1 day'::interval),
                '1 day'::interval
    ...
    WHERE (
            EXTRACT(
                dow
                FROM series_day.series_day
            ) = (demand_window.current_dow)::numeric
        )
),
historical_window_counts AS (
    ...
    FROM (
            historical_windows
            LEFT JOIN public.login_activities ON (
                (
                    (login_activities.success = true)
                    AND (
                        login_activities.created_at >= historical_windows.window_start
                    )
                    AND (
                        login_activities.created_at < historical_windows.window_end
                    )
                )
            )
        )
...
CREATE SEQUENCE public.user_authentication_sources_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
ALTER COLUMN id
SET DEFAULT nextval('public.idp_service_configs_id_seq'::regclass);
--
...
ALTER COLUMN id
SET DEFAULT nextval(
        'public.user_authentication_sources_id_seq'::regclass
    );
--
...
CREATE UNIQUE INDEX index_idp_service_configs_on_connector_active_deleted ON public.idp_service_configs USING btree (connector_id, active, deleted_at)
WHERE (
        (active = true)
