class AddFormUniquenessConstraints < ActiveRecord::Migration[7.0]
  def up
    safety_assured do
      ...
      add_index :hmis_form_definitions, [:identifier, :version], unique: true, name: :uidx_hmis_form_definitions_identifier
      add_index :hmis_form_definitions, [:identifier], unique: true, where: "(status = 'draft')", name: :uidx_hmis_form_definitions_one_draft_per_identifier
      add_index :hmis_form_definitions, [:identifier], unique: true, where: "(status = 'published')", name: :uidx_hmis_form_definitions_one_published_per_identifier
    end
  end
