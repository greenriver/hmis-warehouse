...
class AddTrackNameToHmisSimulationConcurrentEnrollments < ActiveRecord::Migration[7.2]
  def change
    ...
    safety_assured do
      add_column :hmis_simulation_concurrent_enrollments, :track_name, :string unless column_exists?(:hmis_simulation_concurrent_enrollments, :track_name)
      add_index :hmis_simulation_concurrent_enrollments, [:data_source_id, :track_name], if_not_exists: true
    end
  end
end
