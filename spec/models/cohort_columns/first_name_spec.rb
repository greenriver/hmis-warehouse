...
RSpec.describe CohortColumns::FirstName, type: :model do
  let(:user) { create(:user) }
  let!(:hmis_ds) { create(:hmis_primary_data_source) }
  let!(:hmis_user) { create(:hmis_user, data_source: hmis_ds) }
  let!(:source_client) { create(:hmis_hud_client, data_source: hmis_ds) }
  let(:client) { create(:hud_client, FirstName: 'Jamie') }
  ...
  describe '#value' do
    it 'returns the raw first name when the client is not restricted' do
      expect(column.value(cohort_client)).to eq('Jamie')
    end

    it 'redacts the first name when the client is restricted' do
      source_client.mark_as_restricted!(user: hmis_user)
      expect(column.value(cohort_client)).to eq('Name Redacted')
    end
  end
end
