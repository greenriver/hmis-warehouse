...
RSpec.describe CohortColumns::ClientNotes, type: :model do
  ...
  describe '#comments' do
    it 'is not html_safe' do
      GrdaWarehouse::ClientNotes::CohortNote.create!(client: client, note: '<script>alert(1)</script>', user: user)

      expect(column.comments).not_to be_html_safe
    end

    it 'returns the note text verbatim' do
      GrdaWarehouse::ClientNotes::CohortNote.create!(client: client, note: 'Called client', user: user)
