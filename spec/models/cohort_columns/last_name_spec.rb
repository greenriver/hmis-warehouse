...
RSpec.describe CohortColumns::LastName, type: :model do
  let(:user) { create(:user) }
  let!(:hmis_ds) { create(:hmis_primary_data_source) }
  let!(:hmis_user) { create(:hmis_user, data_source: hmis_ds) }
  let!(:source_client) { create(:hmis_hud_client, data_source: hmis_ds) }
  let(:client) { create(:hud_client, LastName: 'Rivera') }
  ...
  describe '#value' do
    it 'returns the raw last name when the client is not restricted' do
      expect(column.value(cohort_client)).to eq('Rivera')
    end

    it 'redacts the last name when the client is restricted' do
