...
RSpec.describe GrdaWarehouse::SystemCohorts::CurrentlyHomeless, type: :model do
  ...
  describe 'finds current clients' do
    ...
    it 'removes the client when they exit' do
      travel_to('2020-02-01'.to_date) do
        ...
        enrollment_1.update(last_date_in_program: '2020-01-31'.to_date, destination: Hud.util.temporary_destinations.first)
        currently_homeless_cohort.sync
        expect(currently_homeless_cohort.cohort_clients.count).to eq(0)
      end
    end
  ...
  describe 'looking at CE enrollments' do
    ...
    describe 'with CLS' do
      let!(:homeless_cls) { create :hud_current_living_situation, InformationDate: date, CurrentLivingSituation: Hud.util.homeless_situations(as: :current).first, EnrollmentID: ce_source_enrollment.EnrollmentID, PersonalID: ce_source_enrollment.PersonalID, data_source_id: ds.id }
      it 'finds the client' do
        travel_to('2020-01-01'.to_date) do
          currently_homeless_cohort.sync
          expect(currently_homeless_cohort.cohort_clients.count).to eq(1)
      ...
      describe 'with additional CLS' do
        let!(:homeless_cls) { create :hud_current_living_situation, InformationDate: date + 1.days, CurrentLivingSituation: Hud.util.other_situations(as: :current).first, EnrollmentID: ce_source_enrollment.EnrollmentID, PersonalID: ce_source_enrollment.PersonalID, data_source_id: ds.id }
