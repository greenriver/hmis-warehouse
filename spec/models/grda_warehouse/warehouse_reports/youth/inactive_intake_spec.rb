...
RSpec.describe GrdaWarehouse::WarehouseReports::Youth::InactiveIntake, type: :model do
  describe '#rows_for_export' do
    let!(:running_user) { create(:acl_user) }
    let!(:role) { create(:role, can_view_youth_intake: true, can_view_client_name: true, can_view_assigned_reports: true) }
    ...
    # An intake with no other activity is only "inactive" if its engagement date falls entirely
    # outside the filter range -- see `InactiveIntake#clients`' `filter.range.overlaps?` check.
    let(:engagement_date) { 3.months.ago.to_date }
    let!(:restricted_intake) { create(:intake, client: restricted_destination_client, engagement_date: engagement_date) }
    let!(:open_intake) { create(:intake, client: open_destination_client, engagement_date: engagement_date) }

    let(:filter) { ::Filters::FilterBase.new(user_id: running_user.id, start: 1.month.ago.to_date, end: Date.current) }
    let(:report) { described_class.new(filter) }

    before do
      Collection.maintain_system_groups
      collection.set_viewables({ projects: [permission_project.id] })
