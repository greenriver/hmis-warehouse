...
RSpec.describe GrdaWarehouse::WarehouseReports::Project::DataQuality::VersionFive, type: :model do
  describe 'project data quality V5' do
    before(:all) do
      ...
      # Create data source, organization, and project
      @data_source = create(:grda_warehouse_data_source)
      @organization = create(:hud_organization, data_source: @data_source, OrganizationID: '3', OrganizationName: 'Test Organization')
      @project = create(:hud_project, data_source: @data_source, OrganizationID: @organization.OrganizationID, ProjectID: '148', ProjectType: 1)
      @range = ::Filters::DateRange.new(start: Date.parse('2016-01-01'), end: Date.parse('2016-12-31'))
      ...
      @clients_sex_female = create_list(:grda_warehouse_hud_client, 10, data_source: @data_source, Sex: 0, FirstName: 'Female', LastName: 'Client', SSN: '123456789')
      @clients_sex_male = create_list(:grda_warehouse_hud_client, 10, data_source: @data_source, Sex: 1, FirstName: 'Male', LastName: 'Client', SSN: '987654321')
      ...
      @clients_sex_not_collected = create_list(:grda_warehouse_hud_client, 3, data_source: @data_source, Sex: 99, FirstName: 'NotCollected', LastName: 'Client', SSN: '333333333')

      # Sex Missing (Sex = nil): 5 clients
      @clients_sex_missing = create_list(:grda_warehouse_hud_client, 5, data_source: @data_source, Sex: nil, FirstName: 'Missing', LastName: 'Client', SSN: '444444444')

      # Name testing: 2 refused, 2 not collected
      @clients_name_refused = create_list(:grda_warehouse_hud_client, 2, data_source: @data_source, Sex: 1, NameDataQuality: 9, FirstName: 'NameRefused', LastName: 'Client')
      @clients_name_not_collected = create_list(:grda_warehouse_hud_client, 2, data_source: @data_source, Sex: 0, NameDataQuality: 99, FirstName: 'NameNotCollected', LastName: 'Client')

      # SSN testing: 3 refused, 10 not collected
      @clients_ssn_refused = create_list(:grda_warehouse_hud_client, 3, data_source: @data_source, Sex: 1, SSNDataQuality: 8, FirstName: 'SSNRefused', LastName: 'Client')
      @clients_ssn_not_collected = create_list(:grda_warehouse_hud_client, 10, data_source: @data_source, Sex: 0, SSNDataQuality: 99, FirstName: 'SSNNotCollected', LastName: 'Client')

      # Collect all clients (total: 20 + 2 + 3 + 5 + 2 + 2 + 3 + 10 = 47 clients)
      all_clients = @clients_sex_female + @clients_sex_male +
                    @clients_sex_refused +
      ...
      @enrollments.first(25).each do |enrollment|
        create(
          :hud_service,
          data_source: @data_source,
      ...
      @enrollments.last(10).each_with_index do |enrollment, index|
