...
RSpec.describe GrdaWarehouse::HiddenClients, type: :model do
  ...
  let!(:inactive_client) { create(:grda_warehouse_hud_client) }
  ...
  before do
    ...
    GrdaWarehouse::InactiveClient.create!(client_id: inactive_client.id, destination_client_id: inactive_client.id, marked_on: Date.current, last_activity_on: 10.years.ago.to_date, retention_years: 7)
  end
  ...
  describe '.not_hidden' do
    it 'keeps only clients that are neither in a restricted identity nor retention-inactive' do
