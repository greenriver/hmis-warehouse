...
RSpec.describe HudDataQualityReport::Archival do
  let(:generators) do
    [
  ...
  before(:all) do
    _ = HudDataQualityReport::Generators::Fy2020::Generator
    _ = HudDataQualityReport::Generators::Fy2022::Generator
  end

  describe 'attachment declarations on HudReports::ReportInstance' do
    it 'declares dq_clients_csv attachment' do
      expect(HudReports::ReportInstance.new).to respond_to(:dq_clients_csv)
    end

    it 'declares dq_living_situations_csv attachment' do
      expect(HudReports::ReportInstance.new).to respond_to(:dq_living_situations_csv)
    end
  end

  describe 'generator registration' do
    it 'registers all DQ generators in HudReportArchival.generator_registry' do
  ...
  describe 'archival_csv_config' do
    ...
    it 'every generator config has exactly the expected attachment keys' do
      expected_keys = [:universe_members_csv, :dq_living_situations_csv, :dq_clients_csv, :report_cells_csv]
      generators.each do |gen|
