...
# mixin with no controller. The request specs (ledger rows L*) can only reach these ops
# through a controller whose earlier gate is closed for the "masked" connector/linkage
# classes (ledger C15), so the caller-discipline cells with no in-method guard —
# null-attached reconcile (M1), the closed setup-email footgun (M1b), and the
# authenticate-only split (M3) — are reachable only here.
#
# Origin: docs/research/idp-jwt-account-management-ledger.md §5 finding 1 (rows M1/M1b/M2/M4).
# Sibling app-vuk covers the authenticate-only class through the request path.
#
# Each context builds one §3 connector/linkage class and lets the *real* ServiceFactory
# resolve it, so the config→service resolution is exercised, not stubbed. Only the remote
# Keycloak calls on the manageable/authenticate-only classes are stubbed, on the memoized
# service instance (Idp::Support#idp_service caches, so the op reuses what we stub).
RSpec.describe Idp::Support, type: :model, if: AuthMethod.jwt? do
  ...
  # active ServiceConfig exists, so the service is a NullService. This is the class the ledger
  # calls out for the surviving caller-discipline cells (M1/M1b): the ops gate on primary_idp
  ...
  context 'null-attached (connector named, config absent)' do
    ...
    # M1b: idp_send_account_setup_email! now self-gates on supports_user_creation? (support.rb:128),
    # so it returns false rather than reaching the raising NullService#send_execute_actions_email.
    # (Also asserted by the shared examples above; called out here as the regression it pins.)
    ...
    # M1: idp_reconcile_email! is the one surviving latent raise — it gates on primary_idp alone
    # (support.rb:144) and reaches NullService#get_user, which raises. The controller can never
    # reach this (change-email tab gates off on supports_email_self_service? == false, §2.4), so
    # this model-layer assertion is the only place it is exercised. Pins that a caller must gate
    # or rescue rather than call it blind on a null-attached user.
  ...
  # logout stay live (ledger M3 / §2.5). The email-adopt half (reconcile_email!) has no capability
  # gate, so it still reaches the admin-API get_user even on this class — the L13 seam app-vuk
  # covers through the request path.
  context 'keycloak-on-file, authenticate-only (manage_users:false)' do
    ...
    it 'idp_reconcile_email! is ungated by capability and still reaches the admin-API get_user (L13 seam)' do
      allow(service).to receive(:get_user).and_return('email' => user.email, 'emailVerified' => true)
