...
RSpec.describe PerformanceDashboards::Overview, type: :model do
  ...
  describe 'race_bucket_titles' do
    it 'returns a hash mapping race keys to labels from HUD utility' do
      titles = report.race_bucket_titles
      expect(titles).to be_a(Hash)
      expected_keys = HudHelper.util.races.keys + ['Multiple']
      expect(titles.keys).to match_array(expected_keys)
      titles.each do |key, label|
        expect(label).to eq(report.race_title(key))
  ...
  describe '#enrolled_by_race' do
    context 'with clients having different race values' do
      let!(:white_client) { create_client_with_race_fields(White: 1) }
      let!(:black_client) { create_client_with_race_fields(BlackAfAmerican: 1) }

      before do
        [white_client, black_client].each do |client|
      ...
      it 'groups clients by race' do
        buckets = report.enrolled_by_race
        expect(buckets).to be_a(Hash)
        expect(buckets['White']).to include(white_client.warehouse_client_source.destination_id)
        expect(buckets['BlackAfAmerican']).to include(black_client.warehouse_client_source.destination_id)
      end
    end
  end

  describe '#enrolled_by_race_data_for_chart' do
    let!(:client) do
      c = create_client_with_race_fields(White: 1)
      enrollment = create_enrollment(
        client: c,
        project: project,
        entry_date: start_date + 10.days,
    ...
    it 'returns data with columns and categories' do
      data = report.enrolled_by_race_data_for_chart
