...
RSpec.describe PerformanceDashboards::Overview, type: :model do
  ...
  describe 'household_bucket_titles' do
    it 'returns a hash mapping household type keys to labels' do
      titles = report.household_bucket_titles
      expect(titles).to be_a(Hash)
      expected_keys = filter.available_household_types.values + [:unknown]
      expect(titles.keys).to match_array(expected_keys)
      titles.each do |key, label|
        expect(label).to eq(report.household_type(key))
  ...
  describe '#enrolled_by_household' do
    context 'with clients in different household types' do
      let!(:adult_only_client) { create_client_with_field(:DOB, start_date - 30.years) }

      before do
        enrollment = create_enrollment(
          client: adult_only_client,
          project: project,
      ...
      it 'groups clients by household type' do
        buckets = report.enrolled_by_household
        expect(buckets).to be_a(Hash)
        expect(buckets.keys).to include(:without_children, :with_children, :only_children, :unknown)
      end
    end
  end

  describe '#enrolled_by_household_data_for_chart' do
    let!(:client) do
      c = create_client_with_field(:DOB, start_date - 25.years)
      enrollment = create_enrollment(
        client: c,
        project: project,
        entry_date: start_date + 10.days,
    ...
    it 'returns data with columns and categories' do
      data = report.enrolled_by_household_data_for_chart
