...
RSpec.describe Role, type: :model do
  ...
  describe 'A Role' do
    it 'provides methods to manage membership' do
      ...
      role.add(user_one)
      expect(role.legacy_users.length).to be 1
      expect(user_one.legacy_roles.length).to be 1

      # can add an array of users
      role.add([user_two, user_three])
      expect(role.legacy_users.length).to be 3
      ...
      expect(role.legacy_users.length).to be 2

      # can remove multiple users
      role.remove([user_one, user_two])
      expect(role.legacy_users.length).to be 0
