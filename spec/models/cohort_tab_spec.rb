...
RSpec.describe GrdaWarehouse::CohortTab, type: :model do
  ...
  describe 'Default tabs generate expected SQL' do
    it 'active query matches' do
      ...
      expect(tab.rule_query(nil, tab.default_rules['Active Clients']).to_sql).to eq(query)
    end
    ...
    it 'housed query matches' do
      ...
      expect(tab.rule_query(nil, tab.default_rules['Housed']).to_sql).to eq(query)
    end
    ...
    it 'ineligible query matches' do
      ...
      expect(tab.rule_query(nil, tab.default_rules['Ineligible']).to_sql).to eq(query)
    end

    it 'inactive query matches' do
      query = '"cohort_clients"."active" = FALSE'
      expect(tab.rule_query(nil, tab.default_rules['Inactive']).to_sql).to eq(query)
    end
  end
end
