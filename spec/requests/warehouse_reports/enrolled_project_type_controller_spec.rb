...
RSpec.describe 'WarehouseReports::EnrolledProjectTypeController', type: :request do
  ...
  let!(:report) { create(:touch_point_report, url: 'warehouse_reports/enrolled_project_type', name: 'Enrolled Project Type') }
  ...
  let!(:she) do
    create(:she_entry, client: restricted_destination_client, project: project,
                       record_type: :entry, project_type: 1, first_date_in_program: 1.month.ago.to_date, last_date_in_program: nil)
  end
  let!(:open_she) do
  ...
  def rendered_workbook
    excel_file = Tempfile.new(['enrolled_project_type', '.xlsx'])
    excel_file.binmode
  ...
  it 'redacts the restricted client name in the HTML view but shows the unrestricted client name' do
    get warehouse_reports_enrolled_project_type_index_path(filter_params)
  ...
  it 'redacts the restricted client name and DOB in the Excel export when the download toggle is on, leaving the unrestricted client intact' do
    ...
    get warehouse_reports_enrolled_project_type_index_path(filter_params.merge(format: :xlsx))
