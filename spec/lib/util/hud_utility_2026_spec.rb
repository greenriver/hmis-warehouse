  ...
  describe 'translation methods' do
    ...
    describe '#race_none' do
      ...
      it 'translates between race_none codes and descriptions' do
        race_none_list['values'].each do |value|
          expect(Hud.util('2026').race_none(value['key'])).to eq(value['description'])
          expect(Hud.util('2026').race_none(value['description'], true)).to eq(value['key'])
        end
      end
    end
    ...
    describe '#ethnicity' do
      it 'returns defined ethnicity descriptions' do
        ethnicities = Hud.util('2026').ethnicities
        expect(ethnicities).to be_a(Hash)
        expect(ethnicities.keys).to include(:hispanic_latinaeo, :non_hispanic_latinaeo, :unknown)

        # Test forward lookup
        expect(Hud.util('2026').ethnicity(:hispanic_latinaeo)).to eq(ethnicities[:hispanic_latinaeo])
        expect(Hud.util('2026').ethnicity(:non_hispanic_latinaeo)).to eq(ethnicities[:non_hispanic_latinaeo])
        expect(Hud.util('2026').ethnicity(:unknown)).to eq(ethnicities[:unknown])

        # Test reverse lookup
        expect(Hud.util('2026').ethnicity(ethnicities[:hispanic_latinaeo], true)).to eq(:hispanic_latinaeo)
        expect(Hud.util('2026').ethnicity(ethnicities[:non_hispanic_latinaeo], true)).to eq(:non_hispanic_latinaeo)
        expect(Hud.util('2026').ethnicity(ethnicities[:unknown], true)).to eq(:unknown)
      end
    end
    ...
    describe '#residence_prior_length_of_stay_brief' do
      it 'translates length of stay codes to brief descriptions' do
        ...
        brief_map.each do |code, brief|
          expect(Hud.util('2026').residence_prior_length_of_stay_brief(code)).to eq(brief)
        end

        # Test reverse lookup for a few values
        expect(Hud.util('2026').residence_prior_length_of_stay_brief('0-7', true)).to eq(10)
        expect(Hud.util('2026').residence_prior_length_of_stay_brief('7-30', true)).to eq(2)
      end
    end
    ...
    describe '#project_type' do
      ...
      it 'translates between project type codes and descriptions' do
        project_types['values'].each do |value|
          expect(Hud.util('2026').project_type(value['key'])).to eq(value['description'])
          expect(Hud.util('2026').project_type(value['description'], true)).to eq(value['key'])
        end
      end
    end
    ...
    describe '#project_type_brief' do
      ...
      it 'translates project type codes to brief descriptions' do
        project_types_brief['values'].each do |value|
          expect(Hud.util('2026').project_type_brief(value['key'])).to eq(value['description'])
        end
      end
    end
    ...
    describe '#living_situation' do
      ...
      it 'translates all types of living situation codes to descriptions' do
        ...
        current_living_situations['values'].each do |value|
          expect(Hud.util('2026').living_situation(value['key'])).to eq(value['description'])
        end
    ...
    describe 'time-related brief methods' do
      describe '#times_homeless_past_three_years_brief' do
        it 'returns brief representations of times homeless counts' do
          expect(Hud.util('2026').times_homeless_past_three_years_brief(1)).to eq('1')
          expect(Hud.util('2026').times_homeless_past_three_years_brief(2)).to eq('2')
          expect(Hud.util('2026').times_homeless_past_three_years_brief(3)).to eq('3')
          expect(Hud.util('2026').times_homeless_past_three_years_brief(4)).to eq('4+')
          expect(Hud.util('2026').times_homeless_past_three_years_brief(8)).to eq('')
          expect(Hud.util('2026').times_homeless_past_three_years_brief(9)).to eq('')
          expect(Hud.util('2026').times_homeless_past_three_years_brief(99)).to eq('')

          # Test reverse lookup
          expect(Hud.util('2026').times_homeless_past_three_years_brief('1', true)).to eq(1)
          expect(Hud.util('2026').times_homeless_past_three_years_brief('4+', true)).to eq(4)
        end
      end

      describe '#months_homeless_past_three_years_brief' do
        it 'returns brief representations of months homeless counts' do
          expect(Hud.util('2026').months_homeless_past_three_years_brief(101)).to eq('0-1')
          expect(Hud.util('2026').months_homeless_past_three_years_brief(105)).to eq('5')
          expect(Hud.util('2026').months_homeless_past_three_years_brief(112)).to eq('12')
          expect(Hud.util('2026').months_homeless_past_three_years_brief(113)).to eq('> 12')
          expect(Hud.util('2026').months_homeless_past_three_years_brief(8)).to eq('')

          # Test reverse lookup
          expect(Hud.util('2026').months_homeless_past_three_years_brief('0-1', true)).to eq(101)
          expect(Hud.util('2026').months_homeless_past_three_years_brief('> 12', true)).to eq(113)
        end
      end
    end
    ...
    describe '#gender (deprecated)' do
      it 'translates between gender codes and descriptions from 2024' do
        Hud.util('2024').genders.each do |key, description|
          expect(Hud.util('2026').gender(key)).to eq(description)
          expect(Hud.util('2026').gender(description, true)).to eq(key)
        end
      end
    end
    ...
    describe '#funding_source' do
      ...
      it 'translates between funding source codes and descriptions' do
        funding_sources['values'].each do |value|
          expect(Hud.util('2026').funding_source_current(value['key'])).to eq(value['description'])
          expect(Hud.util('2026').funding_source_current(value['description'], true)).to eq(value['key'])
        end
      end
    end
    ...
    describe '#ssvf_financial_assistance' do
      ...
      it 'translates between funding source codes and descriptions' do
        ssvf_financial_assistance['values'].each do |value|
          expect(Hud.util('2026').ssvf_financial_assistance(value['key'])).to eq(value['description'])
          expect(Hud.util('2026').ssvf_financial_assistance(value['description'], true)).to eq(value['key'])
        end
      end
    end
    ...
    describe 'funding source constants' do
      it 'maintains critical funding source reference codes' do
        ...
        expect(Hud.util('2026').path_funders).to eq([21])
        expect(Hud.util('2026').local_or_other_funding_source).to eq(46)
        expect(Hud.util('2026').spm_coc_funders).to match_array([2, 3, 4, 5, 43, 44, 54, 55, 56])
      end
    end
  end
  ...
  describe 'categorization methods' do
    describe '#situation_type' do
      it 'categorizes situations correctly based on ID ranges' do
        # Test boundaries and representative values instead of every value
        {
          ...
          test_ids.each do |id|
            expect(Hud.util('2026').situation_type(id)).to eq(expected_type),
    ...
    describe '#destination_type' do
      it 'derives destination type from situation type' do
        expect(Hud.util('2026').destination_type(101)).to eq('Homeless')
        expect(Hud.util('2026').destination_type(204)).to eq('Institutional')
        expect(Hud.util('2026').destination_type(302)).to eq('Temporary')
        expect(Hud.util('2026').destination_type(410)).to eq('Permanent')
        expect(Hud.util('2026').destination_type(8)).to eq('Other')
      end
    end
    ...
    describe 'situation grouping methods' do
      it 'correctly groups situations by their appropriate categories' do
        ...
        situations = {
          homeless: Hud.util('2026').homeless_situations(as: :current),
          institutional: Hud.util('2026').institutional_situations(as: :current),
          temporary: Hud.util('2026').temporary_situations(as: :current),
          permanent: Hud.util('2026').permanent_situations(as: :current),
          other: Hud.util('2026').other_situations(as: :current),
        }
  ...
  describe 'project type helpers' do
    describe '#project_types_with_inventory' do
      it 'excludes types without inventory' do
        ...
        all_types = Hud.util('2026').all_project_types

        expect(Hud.util('2026').project_types_with_inventory).to match_array(all_types - types_without_inventory)
      end
    end

    describe '#homeless_project_type_numbers' do
      it 'returns only homeless project types' do
        homeless_types = Hud.util('2026').homeless_project_type_numbers
    ...
    describe '#permanent_housing_project_types' do
      it 'returns only permanent housing project types' do
        ph_types = Hud.util('2026').permanent_housing_project_types
  ...
  describe 'field name helpers' do
    describe '#race_fields' do
      it 'returns all race field names' do
        expect(Hud.util('2026').race_fields).to include(:AmIndAKNative, :Asian, :BlackAfAmerican,
    ...
    describe '#gender_fields' do
      it 'returns all gender field names' do
        expect(Hud.util('2026').gender_fields).to include(:Woman, :Man, :NonBinary,
  ...
  describe 'CoC methods' do
    ...
    describe '#coc_name' do
      it 'returns CoC name for valid code' do
        coc_codes['values'].sample(10).each do |value|
          ...
          expect(Hud.util('2026').coc_name(code)).to eq(name.strip)
        end
      end

      it 'returns code itself for invalid code' do
        expect(Hud.util('2026').coc_name('INVALID-CODE')).to eq('INVALID-CODE')
      end
    end
    ...
    describe '#valid_coc?' do
      it 'returns true for valid CoC codes' do
        coc_codes['values'].sample(10).each do |value|
          expect(Hud.util('2026').valid_coc?(value['key'])).to be true
        end
      end

      it 'returns false for invalid CoC codes' do
        expect(Hud.util('2026').valid_coc?('INVALID-CODE')).to be false
      end
    end
