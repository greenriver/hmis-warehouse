...
#
# The branch is resolved at *class-load*, so no example can cover both arms and the group is split by
# how the process booted: the Devise half asserts the DeviseUser concern, its macro, and the
# overrides that `super` into the macro; the JWT half asserts the macro is absent and the
# Idp::JwtUser branches are taken.
#
